...
import { ADMIN, ADMIN_BOOTSTRAP_PASSWORD, changePassword, login } from "./api-client";
...
export default async function globalSetup() {
  ...
  console.log("[e2e] 登录 admin（首登）→ 改密（清 MustChangePassword）→ 重新登录拿可用 token...");
  // seed admin 默认 MustChangePassword=true，所有非 /auth/* 路径被中间件拦 403。
  // 改密会自增 token 版本使旧 token 失效，所以必须改密后再登录一次拿新 token。
  // 用 ADMIN_BOOTSTRAP_PASSWORD(changeme) 首登 → 改密到 ADMIN.password（spec 默认用它 login）。
  const bootstrapToken = await login({ username: ADMIN.username, password: ADMIN_BOOTSTRAP_PASSWORD });
  await changePassword(bootstrapToken, ADMIN_BOOTSTRAP_PASSWORD, ADMIN.password);
