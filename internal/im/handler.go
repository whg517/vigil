...
func (h *Handler) callback(c echo.Context) error {
	...
	if !ok {
		return c.JSON(http.StatusNotFound, map[string]string{"error": "unknown platform"})
	}
	...
	if err != nil {
		return c.JSON(http.StatusUnauthorized, map[string]string{"error": "verify failed"})
	}

	// 3. 解析为标准化事件
	evt, err := bot.ParseCallback(payload)
	if err != nil {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "parse failed"})
	}
...
func (h *Handler) handleCardAction(c echo.Context, ctx context.Context, bot IMBot, evt *IMEvent) error {
	...
	if incID == 0 {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "invalid incident_id"})
	}
...
func (h *Handler) handleCommand(c echo.Context, ctx context.Context, bot IMBot, evt *IMEvent) error {
	...
	default:
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "unsupported command: " + cmd})
	}
	if err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": err.Error()})
	}
...
func (h *Handler) handleMention(c echo.Context, ctx context.Context, bot IMBot, evt *IMEvent) error {
	...
	for _, unionID := range evt.MentionAt {
		...
		if _, aerr := h.incSvc.AddResponder(ctx, incID, user.ID, target.ID, imincident.SourceIM); aerr != nil {
			return c.JSON(http.StatusInternalServerError, map[string]string{"error": aerr.Error()})
		}
	}
...
func replyErr(c echo.Context, err error, bot IMBot, channel string) error {
	...
	if errors.Is(err, ErrNotBound) {
		return c.JSON(http.StatusForbidden, map[string]string{"error": "im account not bound, please bind in web"})
	}
