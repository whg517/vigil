...
CI 跑的门禁与维护者本地一致（[ADR-0035](docs/adr/0035-dev-workflow-gates.md)）：commit-lint → 后端 lint→test→build（含 spec/schema drift 检测与覆盖率门禁）→ 前端 lint→build → govulncheck → e2e。任一失败都会阻断合并（红 CI 不予合并）——尤其是覆盖率门禁（45%）、spec/schema drift 检测、e2e 这三项最容易被新贡献者忽略。
