...
[![CI](https://github.com/whg517/vigil/actions/workflows/ci.yml/badge.svg)](https://github.com/whg517/vigil/actions/workflows/ci.yml)
[![License: MIT](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)
[![Go](https://img.shields.io/badge/Go-1.25-00ADD8.svg)](https://go.dev/doc/go1.25)
...
| **IM 原生** | 钉钉/飞书深度协同（本土空白） |
| **AI 原生** | LLM 作为横向 Copilot 贯穿分诊/诊断/复盘 |

## 当前阶段

🚀 **核心功能实现阶段** —— 六大引擎（接入/分诊/排班/升级/通知/IM）、管理面与邮件/webhook/开放 API 全部接入路径已落地；后续演进以 GitHub Issues 跟踪。版本随 git tag（`vX.Y.Z`）发布。
...
>
> 开发模式（本地 `go run` 调试）见 [`CONTRIBUTING.md`](CONTRIBUTING.md) 的 `make` 路线。
...
>
> **若显式设置 `AUTH_ENABLED=false` 或开启上述任一开关（如自行关闭鉴权后暴露公网），切勿将 API 暴露到公网或不受信网络**。仍应通过反向代理 + 网络策略（防火墙/VPN/内网）限制访问。开启时启动日志会打印醒目 WARN。
>
