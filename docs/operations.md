...
```bash
kubectl create secret generic vigil-secrets ...   # ★ 先建 Secret(迁移 hook 与主 Deployment 都引用它)
helm install vigil ./deploy/helm
# 迁移由 pre-install/pre-upgrade hook Job 自动执行(vigil migrate,镜像与主 Deployment 同源):
# hook 成功 → 继续 install/upgrade;hook 失败 → helm 直接报错,新代码不会滚动上线(fail-fast)。
```

- **迁移 hook**(`templates/migrate-job.yaml`,`migration.enabled` 默认开):
  - 首次 install 与每次 upgrade 前自动跑 `vigil migrate`,无需再手动 `kubectl exec`;
...
- `replicaCount > 1` 时 WebSocket 广播依赖 Redis pub/sub(代码已实现),但多副本形态
  未经端到端验证,生产暂不建议。
- Chart 位于 [`deploy/helm/`](../deploy/helm/)(Chart/values/Deployment/Service/Ingress 可选/PDB/迁移 hook Job)。
