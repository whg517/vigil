...
> **注**：**WS 实时看板**与 **Notification 送达查询端点** 中，送达查询（`GET /incidents/:id/notifications`）已在 **T2.2** 落地（见 M2 进度节），本轮 T6.1 无需重复；WS 看板标注为设计目标暂缓（见上表说明）。M6 余下 **T6.2（平台化长尾）/ T6.3（凭据托管）/ T6.4（多副本 WS）** 待后续。
