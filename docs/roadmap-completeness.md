...
> **注**：**WS 实时看板**与 **Notification 送达查询端点** 中，送达查询（`GET /incidents/:id/notifications`）已在 **T2.2** 落地（见 M2 进度节），本轮 T6.1 无需重复；WS 看板标注为设计目标暂缓（见上表说明）。**T6.3（Runbook 执行器凭据加密托管）本轮落地——见上表 T6.3 行**（新增统一 `internal/crypto` AES-256-GCM 机制 + `Credential` 实体 + step `credential_ref` 引用 + 执行时解密注入，并回填 T4.3 工单凭据复用同一加密消除遗留明文缺口）。M6 余下 **T6.2（平台化长尾）/ T6.4（多副本 WS）** 待后续。
