...
## 实现进度（截至 2026-07-05：M0–M6 全部完成）

> 本节随实现推进更新，记录各任务落地状态与关键提交（基线：`main`）。计划正文的任务描述保持不变，作为设计依据。
>
> **总体状态**：**6 个里程碑、39 个任务已全部实现并合入 `main`**，门禁（`go build`/`go test`/`golangci-lint` + `pnpm --dir web build`）与 e2e 全绿。Phase 1（功能可用、生产可交付）完成；Phase 2 设计目标/长尾剩余项见文末「Phase 2 剩余工作」。
...
M1 的 6 个任务已全部闭环合入 `main`：领域事件总线扩展驱动多端同步，closed 终态可达、reopen 重启升级、时间线全程留痕、前端断链修复、错误码归一均已落地。

| 任务 | 状态 | 关键提交 |
|------|:---:|----------|
| T1.1 领域事件总线扩展与多端同步 | ✅ | `bfd02ec`（Incident 全生命周期事件统一发布 —— created/自动升级/自动恢复纳入，驱动 WS 推送 + IM 卡片刷新 + 出站 webhook；自动恢复写时间线且不误解其它活跃单/不解已 acked 单；`timeline_added` WS 广播） |
| T1.2 closed 终态可达 | ✅ | `b28b864`（`POST /incidents/:id/close` + 复盘 publish 回写 `status=closed`，5 态机 closed 终态可达，非法转换 400 `failed_precondition`） |
| T1.3 reopen 重启升级链 | ✅ | `780b0f5`（reopen 后从首层重启升级链、重发通知，权限按附录 A 校验，与 ack 取消 pending 不冲突） |
| T1.4 时间线全程留痕 + IncidentAction 写入 | ✅ | `8254c0a`（补齐零写入的时间线类别 + IncidentAction 写入/查询；REST/Web/IM source 归因区分 api/web/im，不再硬编码 web） |
| T1.5 前端断链修复（json tag） | ✅ | `12529c3`（analytics/Runbook 响应结构体补 camelCase json tag，Dashboard KPI 与 Runbook 执行结果可见；失败步骤保留结构化 Output）；前置 `45c27f2`/`ce756f5` |
| T1.6 处置错误码归一 | ✅ | `fb95c5f`（非法状态转换 Web/IM 均 400、不存在 id 返 404、AI 端点不存在 id 返 404，语义一致） |

### ✅ M2 管理配置真正生效 —— 全部完成

M2 的 8 个任务已全部闭环合入 `main`：管理员在 Web 配置的升级、通知、排班、路由、抑制规则均真正参与运行时评估，配置关联与用户/角色/团队管理端点齐备。

| 任务 | 状态 | 关键提交 |
|------|:---:|----------|
| T2.1 升级引擎补全 | ✅ | `e6aab4e` |
| T2.2 通知分发补全 | ✅（`3907aac`） | 新增 `Notification` ent 实体（送达三态 + suppressed）；`rule.go` 规则评估（condition/channels 参与，B7/C12）；`notifier.go` 逐通道兜底降级链（主通道失败降级，C12）+ 全链失败兜底告警 org_admin（B22）；电话/短信入默认链、`User.phone` 可写（B8）；`GET /incidents/:id/notifications` 送达查询（M13）；模板同名自定义优先消除 Only 歧义（C20） |
| T2.3 排班补全 | ✅（`760fc09`） | 新增 `Override` ent 实体（schedule 边 / user 顶替人 / created_by / start_time / end_time / reason）；`POST\|GET\|DELETE /schedules/:id/overrides` 端点（`schedule.override` 权限，oncall 换己班 / 换他人叠加 `schedule.update`，seed 给 team_admin override）；引擎解算 Override 层（最高优先级、override=true，C5/M8）；空班检测记 metric+Warn+告警 team_admin（无 team 归属兜底 org_admin，C4）；引擎按 `Schedule.type` 差异化解算 + 跳过 `status=disabled` 用户（B21）；`PATCH /schedules/:id` layers 事务重建 Rotation（B21）；oncall 响应结构 `{schedule_id,schedule_name,layers[]}` 加测试锁定（C7） |
| T2.4 路由与抑制增强 | ✅（`91143d5`） | `triage/engine.go` `route()` 增强：slug 直达（向后兼容）→ `Service.labels` 子集匹配（`Event.labels ⊇ Service.labels` + glob `path.Match` + 多命中按匹配标签数具体度优先，确定性裁决）→ Integration 默认 service 兜底（C2/B14）；`suppression.go` 过期规则跳过 + 多命中按 `match_labels` 具体度排序（B15）；`notification/handler.go` SuppressionRule `expires_at` 可设/清除并生效（B15，原 handler 忽略）；新增 `POST /events/:id/reroute` 未路由重路由端点（`triage/handler.go` + `Engine.Reroute`，权限 `service.route_override` 团队软隔离，seed 给 team_admin，M6）；去重/聚合窗口抽为可配 `Triage.{Dedup,Aggregate}Window`（env `VIGIL_TRIAGE_*`，默认 5m，C9） |
| T2.5 配置关联端点 | ✅（`332acb2`） | `service/handler.go` create/update 请求体收 `schedule_ids`/`runbook_ids`（create 全量设置；update 全量替换：nil 不改 / `[]` 清空 / `[x,y]` 替换，`dedupIDs` 去重防唯一约束冲突），`create`/`get`/`update` 响应经 `withAssociations` 回带关联 id（B13/M14）；设置关联须 `service.update`（仅 view 只读角色不得改关联）；escalation_policy 继承已具备（triage `bindPolicyAndPublish` 建单时 `svc.QueryEscalationPolicy()` 绑定），schedule/runbook 为 Service 边、本轮补齐**配置入口**（继承为运行时按边查询，无需拷字段）；`postmortem/handler.go` ActionItem `POST`/`PATCH` 请求体收 `due_date`（RFC3339，schema 有 `due_date` 字段原不收，M14）；OpenAPI spec + 前端 types 重生成（纯增量 +423 行，无 churn） |
| T2.6 用户与密码管理端点 | ✅（`3f3051c`，与 T2.7 同提交） | `auth/handler_user_team.go` 新增 `POST /users` 建用户（username/email/name/timezone/初始密码，权限 `user.create`，`must_change_password=true` 首登强制改密，重复 username/email 归一 409）+ `POST /users/:id/reset-password` 管理员重置他人密码（权限 `user.update`，重置后强制改密 + `AddTokenVersion(1)` 复用 T0.4 吊销机制使被重置用户旧 token 立即失效，M1）；建用户/重置密码落审计（新增 `user.create`/`user.reset_password` action）；`User.phone` 可写已在 T2.2 具备（B8）；wire.go 登记两条敏感路由权限点 |
| T2.7 角色编辑与团队成员管理端点 | ✅（`3f3051c`，与 T2.6 同提交） | `auth/handler.go` 新增 `PATCH /roles/:id` 编辑角色权限集/名称（权限 `role.update`，内置角色 builtin 拒改返 403，权限点逐个校验合法性，重名归一 409，M2）；`auth/handler_user_team.go` 新增 `GET|POST /teams/:id/members` + `DELETE /teams/:id/members/:uid` 成员增删（权限 `team.member.manage` 悬空点落地，S15/M3；handler 内以目标团队 id 作 scope 二次校验实现跨团队拒——`parseTeamScope` 读 `team_id` 不读 `:id`，故补 handler 级软隔离）；移除成员**不联动删 team-scope RoleBinding**（归属与授权正交，撤权走显式 `DELETE /role-bindings/:id`，注释详述）；`createTeamReq`/`updateTeamReq` 加 `parent_team_id`；新增 `role.update`/`team.member.add`/`team.member.remove` action；wire.go 登记路由权限点 + 给 TeamHandler 注入 authz/audit；OpenAPI spec + 前端 types 重生成 |
| T2.8 主资源 team_id 归属修复 | ✅（`96b3976`） | `escalation/handler_policy.go` 与 `runbook/handler.go` 的 `createReq` 补 `team_id`（B26），创建时设 team 归属；新增 `checkCreateTeam`：非 org 级用户经 `VisibleTeamIDs` 校验只能给可管 team 建，跨团队建返 403 且不落库（防越权占位）；顺带修 escalation create 未传 `levels` 时因必填 JSON 字段缺失返 500 的隐患（无 team schema 改动，团队边本已具备）；两包 isolation 测试各加「建己团队 201 且落归属 / 建他团队 403 且不落库」用例；OpenAPI spec（json/yaml/docs.go）+ 前端 types 重生成 |

### ✅ M3 AI 能力补全 —— 全部完成

M3 的 4 个任务已全部闭环合入 `main`：AIInsight 可读持久化与多端呈现、分诊 AI、Copilot 处置推荐、相似检索修复与反馈闭环均已落地（AI 产出带 evidence + human-in-the-loop，写操作 `require_approval` 未审批被阻断）。

| 任务 | 状态 | 关键改动 |
|------|:---:|----------|
| T3.1 AIInsight 可读持久化与多端呈现 | ✅ | `b7457f3` |
| T3.2 分诊 AI | ✅ | `d47e013` |
| T3.3 Copilot 处置推荐 / draft_summary | ✅（`e85d416`） | AIInsight schema 新增 `runbook_suggestion` type（`go generate ./ent/...`）；新增 `internal/ai/copilot.go` `CopilotEngine`（照 T3.2 triage_ai.go 同款：human-in-the-loop + evidence 强制 + 置信度门槛 0.6 + LLM 降级不产出）。**runbook_suggestion**：基于 Incident 内容 + 当前 Service 关联 Runbook 候选 + 相似历史事件的 `runbook_executed` 时间线痕迹，LLM 推荐用哪个 Runbook，产出带 evidence（被推荐 Runbook + 历史处置痕迹）的建议；推荐 id 必须在候选集内（防幻觉）。**★ 安全红线**：`content` 只含 `recommended_runbook_id`/`reason`（供高亮呈现），不含任何执行指令；accept 走 `diagnose.go applyInsight` —— runbook_suggestion 无实际应用动作，终态 `accepted`（非 applied），绝不调用 runbook 引擎；执行仍走 Runbook 两档安全（写操作 `require_approval` 未审批被阻断）。**draft_summary**（stage=copilot）：LLM 草拟 Incident 处理中实时状态摘要（供快速了解/交接），以时间线为 evidence；与 `postmortem_draft`（复盘全文，stage=postmortem，不在本任务）明确区分。新增 `POST /incidents/:id/ai-copilot` 手动触发端点（LLM 不可用返 200 disabled）；wire.go 装配 `CopilotEngine`（共享 GLM provider + 时间线记录器 + 复用诊断链相似检索）。测试锁定：推荐带 evidence、**accept 不绕过 require_approval（含独立跑写步骤验证审批闸门仍生效 + accept 后无 runbook_executed 时间线）**、幻觉 id 过滤、低置信度/无候选不产、draft_summary 生成、与 postmortem_draft 区分、LLM 降级不产不落库。OpenAPI spec + 前端 types 重生成（纯增量 +188/+72 行） |
| T3.4 反馈闭环与知识检索修复 | ✅（`adc8385`） | **知识检索修复（B18，`internal/ai/diagnose.go` FindSimilarPostmortems）**：① 相似复盘检索范围从硬编码 `status='published'` 改为 `IN(published,archived)`——归档复盘仍是有效知识（`knowledgePostmortemStatuses`）；② 补 **LIKE 文本降级**（`findSimilarPostmortemsText`，与 FindSimilar 同款）：pgvector/Embed 不可用时按关联 incident title/summary 关键词匹配 published/archived 复盘（排除自身与 draft/in_review），不再静默返回 `[]`；③ **embedding 补算**（懒计算）：发布时 embedding 计算失败原为完全静默，现 `postmortem/engine.go` 记 Warn，且检索路径 `backfillPostmortemEmbeddings` 按需为 published/archived 缺 embedding 的复盘补算回写（单次至多 10 条，多次渐进，检索库最终一致）；provider/runSQL 不可用时跳过补算（此时走 LIKE 降级，补算无意义）。**AI 反馈闭环**：`internal/analytics` 新增 `AIFeedbackMetrics`（accept/reject/pending 计数 + 采纳率，分母为已改判数不含 pending；ByType 按建议类型细分）+ `GET /analytics/ai-feedback` 端点（`analytics.view` 权限 + team scope 隔离，经 AIInsight→incident→team）。**噪声学习实现边界（不夸大）**：当前实现是「记录 + 可查」——reject 的建议按类型聚合成可观测指标，供运营判断 AI 质量、人工调优 prompt/抑制规则；**不自动改变后续 AI 行为**（不回训、不自动生成 suppression 规则），已在方法注释与本行明确边界。测试锁定：archived 经 LIKE 命中、pgvector 不可用 LIKE 非空、排除自身/draft、embedding 缺失被懒补算、无 provider 不补算、反馈计数/采纳率口径、reject 按类型可查、反馈 team scope 隔离。OpenAPI spec + 前端 types 重生成（纯增量 +271 行，无 churn） |

> **附注（swag 已修复）**：早期记的「swag（OpenAPI 生成器）损坏、spec 靠手工补丁增量维护」已修复（commit `8cbaebe`）——`go generate ./cmd/vigil/...` 正常重生成整份 spec（swag `--v3.1` + swagfix），diff 为纯增量，无需再手工补丁。

### ✅ M4 复盘与协同闭环 —— 全部完成

M4 的 4 个任务已全部闭环合入 `main`：复盘 resolve 自动起草 + 闸门、sections 逐段编辑、Action Item 工单联动、subscriber 定向订阅通知均已落地。

| 任务 | 状态 | 关键改动 |
|------|:---:|----------|
| T4.1 复盘自动起草 + 闸门 | ✅ | `9f49f77`（`OnIncidentResolved` 订阅 IncidentResolved 领域事件——critical 强制自动起草、warning 可配、info 不起草；不回冲人工成果，best-effort） |
| T4.2 复盘 sections 逐段编辑 | ✅ | `d5d869e` |
| T4.3 Action Item 工单联动 | ✅（`466a01f`） | **新增 `TicketIntegration` ent 实体**（`ent/schema/ticket_integration.go`，`go generate ./ent/...`）：出向工单集成配置（type webhook/jira/zentao、endpoint、credential[`.Sensitive()` 加密不回显，复用 Integration.token 模式]、config[目标项目/字段映射]、team 归属[nil=org 级]）。**新增 `internal/ticket` 包**：`adapter.go`（`Adapter` 接口 + **通用 webhook 适配器**[POST 可配 URL，payload 含 ActionItem，凭据走 `Authorization: Bearer` 头不进 body/日志，带 SSRF 防护连接时校验真实 IP 防 rebinding，与 runbook 出站同款]+ **Jira/禅道预留适配器**[`NewJiraAdapter`/`NewZentaoAdapter` 占位返回 `ErrAdapterNotImplemented`，建单降级不阻断]）；`engine.go`（`Engine.OnPostmortemPublished`：为复盘下**未建单**[tracker_url 为 NULL/空]的 ActionItem 建外部工单回写 tracker_url，team 级集成优先、org 级兜底；`SyncStatus`：ActionItem→done 单向同步工单侧[仅通用 webhook 承载，Jira/禅道 no-op]，**单向**——不反向拉工单状态[工单侧回写是设计目标非本轮]）；`handler.go`（`/ticket-integrations` CRUD，凭据 Sensitive 不回显 + SEC-01 team 软隔离 + 建单鉴权 team 级用户须给可见 team 建 + 审计留痕）。**建单触发点**：复盘 `Transition(→published)` 经新增 `postmortem.TicketCreator` 接口调 `OnPostmortemPublished`（接口解耦，postmortem 不反向依赖 ticket 包，与 IncidentCloser 同款）；**best-effort**：工单系统不可达/未实现/失败均不阻断复盘发布（引擎内部吞错记日志）。**公共文件改动**：ent schema（Team 加 `ticket_integrations` 反向边）；`auth/permission.go`（新增 `ticket_integration.{view,create,update,delete}` 4 权限点入 `AllPermissions`）；`auth/audit.go`（新增 3 审计 action）；`auth/scope.go`（注册 `ticket_integration` 资源反查）；`auth/seed.go`（team_admin 加 4 权限，org_admin 经 allPermissions 自动含）；`server/wire.go`（装配 ticket 引擎[webhook + jira/zentao 预留适配器，SSRF 生产禁私网]+ handler + 敏感路由权限点 + postmortem 注入 TicketCreator/TicketSyncer）。**凭据托管**：`credential` 字段 `.Sensitive()` → `json:"-"`（API 响应恒不回显）+ String() 输出 `<sensitive>`（日志脱敏）；明文仅进程内传给适配器建单。测试锁定：配置后发布 → ActionItem 建单回填 tracker_url（team/org 两级解析）、不可达时发布不阻断且不回填、未实现适配器降级、已建单跳过不重复、凭据不进 JSON/String()/请求 body、SSRF 生产拦元数据地址、单向 done 同步。OpenAPI spec + 前端 types 重生成（纯增量）。**注**：complete Jira/禅道 SDK 未做（体量大），先通用 webhook + 预留接口。 |
| T4.4 subscriber 定向订阅通知 | ✅（`f3cae2a`） | **新增 `Subscription` ent 实体**（`ent/schema/subscription.go`，`go generate ./ent/...`）：定向订阅关系（`user` 拥有者[必填] + `team`/`service` scope[二选一，均 nullable] + `channels` 通道偏好[有序降级链，空=默认链] + `min_severity` 最低告知阈值[critical>warning>info，默认 warning 屏蔽 info 噪音]）；User/Team/Service 各加 `subscriptions` 反向边。**新增 `internal/subscription` 包**：`notifier.go`（`Notifier.Subscribe` 订阅 Incident 全生命周期领域事件[created/acked/escalated/resolved/closed/reopened/responder_added]；`OnIncidentEvent` 按 incident 的 team/service 反查订阅者 → 交通知链定向送达；**按订阅者去重**[同一人订了 team 又订了 service 只发一次]；**min_severity 过滤**[低于阈值不告知]；**禁用订阅者跳过**[status!=active 不发，与 T0.3 一致]；best-effort 不阻断事件派发）；`handler.go`（`GET|POST|DELETE /subscriptions`，登录用户管理**自己**的订阅——list 只回自己的、delete 按 (id,user) 双条件[他人的 404 不泄露存在性]；create 校验 team/service 二选一 + **可见性校验**[订阅 scope 须对用户可见，防越权窥探其他团队 Incident 摘要，org 级用户全可见/team 级须持该 team 绑定]）。**定向通知如何复用 T2.2**：`notification/notifier.go` 新增 `NotifyTargeted(inc, targets, channels)` —— 复用 T2.2 全套分发（规则评估 channels/template/quiet_hours、逐通道降级链、送达三态落库），差异仅两点：① 收件人来源是订阅关系（只读干系人）而非升级链 target；② 订阅者**一律按非值班人判定**（isOncall=false）→ **quiet_hours 对非 critical 夜间通知生效**（记 suppressed，与 E.6 语义一致），critical 穿透照发。不聚合（订阅告知即时性优先）。**公共文件改动**：ent schema（User/Team/Service 反向边）；`notification/notifier.go`（新增 `NotifyTargeted`，复用既有 `deliverChain`/`recordDelivery`/规则/模板解析）；`server/wire.go`（装配订阅 Notifier 挂事件总线 + 订阅 handler 注入 authz）。**无新增权限点**（订阅是用户自管偏好，任何登录用户管理自己的，不挂 RouteGuard 敏感路由）。测试锁定：订阅 team/service 后该 Incident 事件触发定向通知、team+service 双订阅去重只发一次、min_severity 过滤 info、禁用订阅者跳过、`NotifyTargeted` 送达走送达记录、quiet_hours 抑制非值班订阅者夜间非 critical（记 suppressed）、critical 穿透 quiet_hours、订阅端点自管（列自己/删自己 204 删他人 404/二选一校验/不可见团队 403 不落库）。OpenAPI spec + 前端 types 重生成（纯增量）。 |

### ✅ M5 接入与集成补全 —— 全部完成

M5 的 5 个任务（T5.1–T5.5）已全部闭环合入 `main`：开放 API 投递 + 接入运维、RawEvent 查询/重放 + 自动回灌、出站 webhook 签名 + 死信、Runbook trigger 求值、IM 平台补全、IM 绑定自助/解绑均已落地。

| 任务 | 状态 | 关键改动 |
|------|:---:|----------|
| T5.1 开放 API 投递 + 接入运维 | ✅（`e31b2b7`，与 T5.5 同提交） | **开放 API（`POST /api/v1/events`）**：外部系统凭 `X-Vigil-Key`（复用既有 resolver 身份链路）程序化投递 Event。`internal/ingestion/handler_events.go` 新增 `deliverEvent`——把 webhook 的「落 RawEvent → 限流/背压 → 归一化队列 → 分诊」核心抽为共用 `Handler.ingest`（webhook 与开放 API **共用同一归一化链路不重复**），payload 走目标接入点 type 对应适配器（通用 JSON 接入点用 `GenericJSONAdapter`）。目标接入点由 `integration_id`（query 优先，回退 body 顶层）指定；**幂等**由归一化落 Event 的 `(source, source_event_id, status)` 唯一索引兜底（重复 source_event_id 不产新 Event，与 webhook 一致）；限流/背压 payload 仍落库不丢（标记 `requeued` 待回灌）；返回 **202** + `raw_event_id`。**鉴权**：新增权限点 `event.create`——登录/API Key 用户须对**目标接入点**有该权限（团队软隔离，`scope.go` 复用 `integration` 反查 team），防凭 API Key 往任意接入点灌告警；禁用接入点投递统一 **401**（C10）、不存在 404。**接入运维（`internal/integration/handler.go`）**：① `POST /integrations/:id/test` 干跑——样例 payload 走**同一适配器注册表**（`SetAdapterRegistry` 注入 `ingestion.AdapterRegistry`）归一化验证 labels/severity 命中，返归一化预览，**绝不落 RawEvent/Event/建单**（纯预览）；② `POST /integrations/:id/rotate-token` 轮换——生成新 token 旧立即失效（`ActionIntegrationRotateToken` 留痕），两端点均需 `integration.update`。**公共文件改动**：`auth/permission.go`（+`event.create`/`raw_event.view`/`raw_event.replay` 入 `AllPermissions`）；`auth/audit.go`（+`integration.rotate_token`/`raw_event.replay` action）；`auth/scope.go`（注册 `raw_event`→integration→team 反查）；`auth/seed.go`（team_admin 加 3 权限，org_admin 经 allPermissions 自动含）；`server/wire.go`（ingest handler 注入 authz/scope + 注册 `/events`；integration handler 注入 adapterRegistry；敏感路由权限点登记）。测试锁定：投递落 RawEvent 返 202、integration_id 从 query/body 解析、缺失 400、未知接入点 404、禁用 401、端到端幂等（两次投递→两条 RawEvent→归一化后 1 条 Event）、干跑预览命中 labels/severity、干跑不落库、干跑解析失败可见、token 轮换旧失效。OpenAPI spec + 前端 types 重生成（纯增量）。 |
| T5.3/T5.5 RawEvent 查询/重放 + 自动回灌 | ✅（前轮） | **RawEvent 运维端点（`internal/ingestion/handler_rawevents.go`）**：① `GET /raw-events?integration_id=&status=&limit=`——按接入点/状态过滤，返回 `received/normalized/parse_failed/requeued` **状态分布计数**（`counts`）+ 明细（`items`，不回显原始 payload——可能含敏感内容，明细排障用 error/headers），**团队软隔离**（team 级用户仅见本团队接入点 raw_event，经 `integration.team` 过滤）；status 非法枚举返 400。② `POST /raw-events/:id/replay`——把 parse_failed/requeued/received 的 raw_event 重置回 `received` 并**复用 ingest 的 `enqueueNormalize`** 重新投入归一化（**幂等**由 Event 唯一索引兜底，重复重放不产重复 Event）；资源级鉴权 `raw_event.replay`（经 raw_event→integration→team 反查）+ 重放留痕。**超限自动回灌（`internal/ingestion/sweeper.go`）**：`RequeueSweeper` goroutine ticker（与 `runAggregationFlusher` 同款，纳入优雅关闭 `Wired.Closers`），周期（默认 30s，batch 50）扫 `requeued` 的 raw_event（限流/背压/入队失败落库者）重置回 received 并入队——过载/故障恢复后**自动补投**无需人工逐条重放；入队仍失败者回滚回 requeued 下轮再试。**行为微调**：`Handler.ingest` 现把限流/背压/入队失败的 RawEvent 统一标记 `requeued`（原限流/背压保持 received），使其被巡检回灌纳管（原仅入队失败标 requeued）。**无 schema 改动**（RawEvent 状态枚举已含 requeued）。测试锁定：查询状态分布计数 + 过滤、非法 status 400、重放重置回 received 清 error 并入队、重放不存在 404、巡检把 requeued 批量回灌为 received 且不触碰 normalized。 |
| T5.2 出站 webhook 签名 + 事件补全 + 死信 + WS 覆盖 | ✅（`14b663c`） | **S13 出站签名（`internal/webhook/dispatcher.go`）**：非空 `VIGIL_WEBHOOK_SIGNING_SECRET`（`config.Webhook.SigningSecret`）时每次出站 POST 加 `X-Vigil-Signature`（`hex(HMAC-SHA256(secret, timestamp + "." + body))`）+ `X-Vigil-Timestamp`——时间戳纳入签名基串，接收端用同一密钥重算验源、按容忍窗口判时效**防重放**；未配则不签（**向后兼容**，注释标注生产强烈建议配）。导出 `Sign()` 供接收端/测试对拍同一算法。**C24 事件补全（核实：T1.1 已覆盖，本轮无需补代码）**：`server/wire.go:236-248` 已把出站 dispatcher + WS hub 订阅到全部 7 类生命周期事件（含 `IncidentCreated`、自动升级 `IncidentEscalated`——后者经 `escEngine.SetBus(bus)` 计时器到点也发事件）；`dispatcher_test.go` 已有 created/escalate 出站用例。**WS 推送覆盖（核实：已全覆盖）**：`wsHub.OnIncidentEvent` 订阅 created/acked/resolved/escalated/reopened/closed/responder_added 全 7 类 + `timeline_added`（经 `timelineRecorder.SetBroadcaster(wsHub)`）——无遗漏，本轮无需补。**C24 死信（新增 `WebhookDelivery` ent 实体）**：出站全部重试耗尽仍失败不再静默丢弃，改落 `WebhookDelivery(status=failed)` 死信（成功也落一条便于送达率统计），留存 url/event/incident_id/**payload 全量**（重放原样重发）/attempts/last_error/last_status_code。归属取舍：出站 URL 是全局配置式订阅（非 team 资源），实体**不挂 team 边**，查询/重放走 **org 级权限点**（`webhook_delivery.view`/`webhook_delivery.replay`，不走团队软隔离）。`internal/webhook/recorder.go`（`EntRecorder` 实现 `DeliveryRecorder` 接口——包定义接口、装配层用 ent 实现，dispatcher 不直接持 `*ent.Client`，best-effort 记录失败不阻塞出站）。**死信运维端点（`internal/webhook/handler.go`）**：`GET /webhook-deliveries?status=&incident_id=&limit=`（状态分布 counts + 明细，不回显 payload；非法 status 400）+ `POST /webhook-deliveries/:id/replay`（复用 `Dispatcher.SendOnce` 出站签名同步单发；回写 attempts 累加/状态翻转/错误码；成功 200 失败 502 让调用方知晓未送达；重放留痕 `webhook_delivery.replay`）。**公共文件改动**：ent schema 新增 `webhook_delivery.go`（`go generate ./ent/...`）；`config.go`（Webhook.SigningSecret）；`auth/permission.go`（+`webhook_delivery.{view,replay}` 入 `AllPermissions`，org_admin 经 allPermissions 自动含，team_admin **不含**——全局运维）；`auth/audit.go`（+`webhook_delivery.replay` action）；`httputil/dto.go`（AckResponse 加通用 `id` 字段）；`server/wire.go`（dispatcher 注入签名密钥 + EntRecorder + 注册死信 handler + 敏感路由权限点）。测试锁定：配密钥出站带签名头且同算法可验源/错误密钥不通过、未配密钥不签、重试耗尽落死信（payload/错误/状态码可查）、成功也落记录、`SendOnce` 成功/失败反馈、死信查询分布+过滤+非法 status 400、重放成功回写 success+attempts+清错、重放仍失败 502+记录保持 failed、重放不存在 404。OpenAPI spec + 前端 types 重生成（纯增量）。 |
| T5.3 Runbook trigger 求值与自动展示 | ✅（`eea429b`） | **B13 补齐**：Runbook `trigger`（manual/on_incident/on_severity/on_label_match）此前可存但不求值（仅手动 execute）、`IsReadOnly` 无调用方（auto-run 未实现）。本轮接通求值路径：`internal/runbook/trigger.go` 新增 `TriggerEvaluator` 订阅 `IncidentCreated`，求值该 Incident 所属 Service 关联 Runbook 的 trigger——on_incident（建单即命中）、on_severity（severity ≥ 阈值，critical>warning>info 有序比较，阈值取 `trigger.severity` 或 `trigger.condition` 文本里的严重度词，无法解析保守命中）、on_label_match（`trigger.labels` 为 Incident 聚合 Event labels 并集的子集则命中）。命中结果=**自动「展示」关联 Runbook**（写 `runbook_suggested` 时间线，`timeline.Recorder.RecordRunbookSuggested`，Web/IM 可见、经 B11 WS 广播实时刷新），**而非自动执行**。**★ auto-run 安全红线**：新增 Runbook schema `auto_run` 字段（默认 false）；自动执行**仅**当 `auto_run=true` 且 `IsReadOnly(rb)==true`（全步骤只读诊断）时才复用 `Engine.Execute`（`approved=false`，写步骤守卫仍在作第二道防线）；**含任一写步骤的 Runbook 即使配 `auto_run=true` 也绝不自动执行——只展示**（`evaluateOne` 硬守卫 `rb.AutoRun && IsReadOnly(rb)`）。求值纯函数抽 `trigger_match.go` 便于单测。**配置入口**：runbook handler `createReq`/`updateReq` 收 `auto_run`（PATCH 用指针，nil 不改）。**公共文件改动**：`ent/schema/runbook.go`（+`auto_run` 字段）+ `ent/schema/timeline_action.go`（TimelineItem type 枚举 +`runbook_suggested`），`go generate ./ent/...`；`internal/timeline/recorder.go`（+`RecordRunbookSuggested`）；`internal/runbook/handler.go`（create/update 收 auto_run）；`internal/server/wire.go`（`NewTriggerEvaluator` 订阅 `IncidentCreated`）；OpenAPI spec + 前端 types 重生成（纯增量）。测试锁定：on_incident/on_severity（命中+不命中）/on_label_match（命中+不命中）求值并展示；auto_run+全只读诊断自动执行（诊断端点被调用）；**含写步骤即使 auto_run 也绝不自动执行、写端点零调用、autoRunnable=false**；引擎守卫双保险（approved=false 写步骤跳过标待审批）；未路由无 Service 不展示；manual 不自动触发；纯匹配函数表驱动全类型覆盖。 |
| T5.4 IM 平台补全（钉钉/企微 + CardStore） | ✅（`e053552`） | **B16 钉钉卡片降级**：钉钉 `sampleActionCard` 机器人消息无原地更新能力，`UpdateCard` 原为 no-op（卡片永停下发态）。本轮实现降级——`SendCard` 把目标 channel 编入返回的 cardID（`channel|msgID`，`encodeCardID`/`decodeCardID`），`UpdateCard` 解出 channel 向**同群重发一条带最新状态徽章的新消息**（「⚠️ INC-xxx 已确认 by 张三」），群内可见状态变更；无 channel 编码（历史裸 msgID）返回 `im.ErrCardUpdateNoChannel`（best-effort 让调用方感知降级未成，不阻塞主流程，状态已落库）。**B16 钉钉 @人解析**：`ParseCallback` 解析回调 `atUsers`（staffId 优先、退回 dingtalkId，排除操作者本人），@人拉入与飞书对齐；钉钉权限限制下 staffId 缺失需以 dingtalkId 绑定、两者皆空则如实跳过（注释标注限制）。**顺带修正**（B16 关联）：`handleMention` 原先在鉴权前未回填 incident id → team scope 为 nil，持团队级 `add_responder` 者被误拒；改为**先解析 incident 再回填 evt.IncidentID 再鉴权**，team scope 正确解算（飞书/钉钉 mention 同受益）。**B24 CardStore 持久化**：incident→卡片 ID 映射从进程内存 map 抽为 `CardStore` 接口（`internal/im/cardstore.go`），提供 `MemoryCardStore`（降级兜底）+ `RedisCardStore`（HASH `vigil:im:card:{id}` + 7 天 TTL，复用项目 Redis），进程重启后已发卡片仍可 `UpdateCard` 刷新；接口方法带 ctx，handler/refresher/channel 依赖接口，`wire.go` 按 Redis 是否可用选实现。**B17 值班群未配可观测**：`notification_channel.go` 原在 `VIGIL_IM_ONCALL_CHANNEL` 未配时静默 `return nil,nil`（无 metric/无 log 盲区）；改为**有可用 IM bot 时**记 `vigil_im_oncall_channel_missing_total`（新增 metric）+ Warn 日志，通知仍返回 nil error 交上层降级链走其它通道兜底（C12，不阻断）。**企微**：仍 `NoopBot` 占位（完整适配器待 PoC，是设计目标）；因 `Available()==false` 被排除，IM 对企微不发但**不静默丢告警**——走 notification 逐通道兜底降级链（邮件/电话/短信），全链失败才兜底告警 org_admin；noop.go/wire.go 明确注释+日志标注不静默丢失保证。**公共文件改动**：`internal/metrics/metrics.go`（+`IMOncallChannelMissing`）；`internal/im/bot.go`（+`ErrCardUpdateNoChannel`）；`internal/server/wire.go`（CardStore 按 Redis 选实现 + IMChannel 注入 logger + wecom 占位日志）。测试锁定：钉钉 UpdateCard 降级重发（群发接口再被调）、裸 cardID 返 ErrCardUpdateNoChannel、cardID 编码 channel、钉钉 mention 解析（staffId/dingtalkId 退回/排除本人）、mention 端到端 AddResponder（团队权限不再误拒）、CardStore Redis 重启后可读（miniredis 双实例模拟）、多平台独立、nil client 降级、接口一致性。 |
| T5.5 IM 绑定自助/解绑 | ✅（`e31b2b7`，与 T5.1 同提交） | **M11 解绑端点**：新增 `DELETE /users/:id/im-accounts/:platform`（`internal/auth/handler_user_team.go` `unbindIMAccount`），误绑可纠正。**鉴权「本人或 admin」**：本人（登录态 user_id==:id）恒可自助解绑；他人须持 `user.im.bind`（authz 未注入的测试装配下保守拒绝跨用户解绑）——**不走 RouteGuard 单权限点**（否则强制 admin-only，本人无法自助），在 handler 内做复合判定。**解绑双删**：`mapper.UnbindAccount` 删独立表 `IMAccountBinding`（按 user+platform）+ JSON 字段幂等移除（双写路径对称），无绑定返 `removed=false` → handler 判 404（幂等语义无可删对象）。**审计**：新增 `user.im_unbind` action，解绑落审计（IM 账号是鉴权桥梁，误绑/恶意解绑须可追溯）。**装配**：`wire.go` 给 UserHandler 注入 `SetIMAccountUnbinder(imMapper)`。**注**：前端绑定/解绑页未做（web/src，属旅程 B.9 UI，本轮聚焦后端端点齐备；标注为后续）。OpenAPI spec + 前端 types 重生成（纯增量，+`DELETE .../im-accounts/{platform}`）。测试锁定：本人解绑 204+调用+审计、无绑定 404、无 authz 解他人 403 且不调 unbinder、admin 持 user.im.bind 解他人 204、mapper 双删（独立表+JSON）解绑后不可解析、仅 JSON 历史数据也能解绑、无绑定 removed=false。 |

> **注**：任务提示所称「T5.5 raw_event 重放」对应本文 roadmap 的 **T5.3**（M12），前轮已落地。roadmap 原 **T5.5（IM 绑定自助/解绑）** 本轮落地——见上表 T5.5 行。**T5.4（IM 平台补全）** 本轮落地——见上表 T5.4 行。**T5.3 Runbook trigger 求值**（B13，独立于 M12 RawEvent 重放）前轮落地。至此 M5 五个任务（T5.1–T5.5）全部完成。

### ✅ M6 报表 / 平台化 / 长尾 —— 全部完成

M6 的 4 个任务已全部闭环合入 `main`：报表 CSV 导出 + 定时聚合快照、平台化长尾（保留清理/服务拓扑/集成模板）、Runbook 执行器凭据加密托管、多副本 WebSocket pub/sub 均已落地。

| 任务 | 状态 | 关键改动 |
|------|:---:|----------|
| T6.1 报表导出 + 定时聚合 + 看板数据源 | ✅（`2fbea67`） | **报表 CSV 导出（M15.6，`internal/analytics/export.go`）**：alerts/incidents/team-load/postmortems 各加 `GET /analytics/{维度}/export` 端点，`encoding/csv` 输出 + `Content-Disposition: attachment` 附件下载；复用现有维度查询与 `resolveScope`——权限同 `analytics.view`、team 软隔离一致（team 级用户导出只见本团队数据、org 级看全组织）；incident 导出含 severity/status 分布（按 key 排序保证 CSV 稳定可 diff）、team-load 每团队一行。**定时聚合快照（M13，新增 `MetricsSnapshot` ent 实体 + `internal/analytics/aggregator.go`）**：`go generate ./ent/...` 新增 `metrics_snapshot`（period hourly/daily、period_start/end、各维度指标定格、by_severity/by_status JSON、team 边 nullable[挂 team=团队级快照 / NULL=org 全局]，Team 加 `metrics_snapshots` 反向边）。`Snapshotter.Aggregate` 对 org 全局 + 每团队各预计算一份快照（复用 Engine 6 维度查询，聚合「上一个完整窗口」避免定格进行中窗口），幂等[同 (team,period,period_start) 先删后建，NULL team 显式删兜底去重]。**触发双路径**：① Asynq 低优先级 `TaskAggregate` worker（`HandleTask`，可外部编排触发）；② 装配层 ticker goroutine（`Run`，与 sweeper 同款、纳入优雅关闭 `Closers`，每小时聚合 daily 快照）——二者调同一 `Aggregate` 幂等互不冲突，无需外部 cron 也能定期聚合。**端点可选读快照**：alerts/incidents/postmortems 加 `?source=snapshot`——命中快照快返、无快照降级实时；**默认 realtime 保准确**（快照是加速旁路非唯一真相）；快照读取按 scope 隔离（org 读 NULL-team 全局快照 / 单团队读该团队 / 多团队无法合并降级实时）。**WS 看板**：本轮**未做**（标注为设计目标，能力域 15 §B3「实时看板 WebSocket 推送」）——现有 WS hub 为 per-incident 订阅，看板需新增 dashboard 广播 topic + 前端订阅，体量另计；报表导出 + 定时聚合已满足 T6.1 验收①②③（可导出/有聚合数据源/指标可查）。**公共文件改动**：ent schema（`metrics_snapshot.go` + Team 反向边，`go generate`）；`server/wire.go`（装配 Snapshotter + 注册 TaskAggregate worker + 启动聚合 ticker + handler 注入 Snapshotter + 4 条导出路由挂 `analytics.view` 权限点）。**无新增权限点**（导出复用 `analytics.view`，快照聚合是后台任务无端点）。OpenAPI spec + 前端 types 重生成（纯增量 +4 导出端点）。测试锁定：CSV 附件头 + 表头/数据行格式、导出 team scope 隔离（团队级只见本团队 total、team-load 只列本团队一行）、聚合写 org+每团队快照、快照与实时同窗口同 scope 口径一致（含 severity 分布）、重跑幂等不产重复行（org NULL-team 恰 1 行）、读快照 scope 隔离（org/单团队命中、多团队/空 scope 降级 nil）、无快照降级、source=snapshot 命中快照 vs 默认走实时。 |
| T6.2 平台化长尾（保留清理/服务拓扑/集成模板） | ✅（`2b5f311`） | **Event/RawEvent 保留清理（最有价值，M15，新增 `internal/event/retention.go`）**：Event 是海量不可变原始信号（只追加），无保留策略长期堆积吃存储。`RetentionSweeper` 按可配保留期（env `VIGIL_RETENTION_EVENT_DAYS` 默认 90 / `VIGIL_RETENTION_RAW_EVENT_DAYS` 默认 30，<=0 表示对应类不清理向后兼容）周期删旧记录释放存储。**★ 安全三条**：① **保护活跃 Incident 证据**——只删「关联 Incident 已 closed 或无关联」的 Event（谓词 `Or(Not(HasIncident()), HasIncidentWith(status=closed))`），关联未 closed（triggered/escalated/acked/resolved 任一活跃态）的 Event 即使超期也不删；② **批量分页**（`VIGIL_RETENTION_BATCH_SIZE` 默认 500，取一批 id 再按 id 删，删满继续下一批直到不足一批），避免一次删百万行的大事务锁表；③ RawEvent 独立更短保留期且**不删 requeued**（待回灌，删了丢告警）。**触发双路径**（照 T5.5 RequeueSweeper/T6.1 aggregator 同款）：装配层 ticker goroutine（默认 6h，纳入优雅关闭 `Closers`）+ Asynq 低优先级 `TaskCleanup` worker（可外部编排），二者调同一 `Sweep` 幂等。**服务拓扑 depends_on（M4.4，schema+CRUD+一层查询）**：Service schema 加自引用边 `depends_on`（依赖的下游）+ 反向边 `dependents`（依赖本服务的上游=影响面），`go generate ./ent/...`；handler create/update 收 `depends_on_ids`（全量替换语义，**剔除自引用**防自环），`withAssociations` 回带；新增 `GET /services/:id/dependencies` 一层依赖查询（返回 depends_on 下游 + dependents 影响面，精简节点 id/name/slug/status）——**不做完整拓扑算法/环检测**（标注设计目标）。**集成配置模板（M14.6 向导后端辅助，新增 `internal/integration/config_template.go`）**：`GET /integrations/config-template?type=prometheus` 返回该类型配置字段说明 + 上游接线指引（webhook URL 模板等），覆盖 schema 枚举全集 7 种；缺 type/all 返全部，未知 type 404；只读无副作用不挂资源鉴权。**完整分步向导 UI 标注设计目标**（前端大件）。**Asynqmon（部署配置）**：docker-compose 加 asynqmon service 配置块（默认注释，`hibiken/asynqmon` 观测异步队列，注明生产须内网/加鉴权）。**值班大屏/PWA、WS 实时看板标注设计目标**（纯前端/部署大件，不写空壳）。**公共文件改动**：ent schema（`service.go` 加 depends_on 自引用边，`go generate`）；`config.go`（新增 `Retention` 配置 + `EffectiveInterval`/`EffectiveBatchSize`）；`server/wire.go`（装配 RetentionSweeper + 注册 `TaskCleanup` worker + ticker 启动纳入 Closers）；`.env.example`（保留清理 4 项 env）；`docker-compose.yml`（asynqmon 可选服务）。**无新增权限点**（保留清理是后台任务无端点；depends_on 复用 `service.view/update`；配置模板只读）。OpenAPI spec + 前端 types 重生成（纯增量 +dependencies 端点）。测试锁定：**Event 清理**——超期无关联删/未超期留、**保护活跃 Incident 的 Event（关联 closed 才删）**、RawEvent 按龄清理且不删 requeued、保留期<=0 不清理、batch 小于总量多批删净；**服务拓扑**——create/update 收 depends_on_ids、全量替换、自引用剔除、`/dependencies` 返回 depends_on+dependents（影响面）、不存在 404；**集成模板**——按 type 返模板、缺 type 返全集 7 种、未知 type 404。 |
| T6.3 Runbook 执行器凭据加密托管 | ✅（`aad6a9c`） | **统一加密机制（新增 `internal/crypto` 包，AES-256-GCM）**：`Cipher.Encrypt/Decrypt` —— AEAD 同时保证机密性+完整性（抗篡改），密文格式 `base64(nonce‖ciphertext‖tag)`、每次随机 nonce；密钥从 env `VIGIL_CREDENTIAL_ENCRYPTION_KEY`（base64/hex 编码的 32 字节，`openssl rand -base64 32` 生成，**绝不硬编码**）；空密钥→`ErrKeyNotConfigured`（装配层降级）、格式非法→`ErrInvalidKey`、解密失败→泛化 `ErrDecrypt`（不泄露原因防侧信道）。★ 本包是**唯一**凭据加密实现，Runbook 凭据与工单凭据都复用它（避免两套加密）。**新增 `Credential` ent 实体**（`ent/schema/credential.go`，`go generate ./ent/...`）：`name`/`type`(bearer/token/basic/header)/`secret_ciphertext`[`.Sensitive()`→`json:"-"`，存密文不回显]/`config`/team 归属[nil=org 级]；Team 加 `credentials` 反向边。**Runbook step 引用凭据（S16 核心）**：`schema.StepTarget` 加 `credential_ref` 字段（凭据 id，替代把 Ansible/Jenkins token 明文塞 endpoint/params）；`internal/runbook/credential.go` `EntCredentialResolver` 反查+解密+按 type 组装 HTTP 头（bearer→`Authorization: Bearer`、token→原样、basic→`Basic`、header→自定义头名）；`executor.go` HTTPExecutor/InternalExecutor 持 `CredentialResolver`，`Execute`/`checkHTTP` 在**执行时**解密注入头——**★ 明文只在 `req.Header.Set` 一瞬间存在于内存，绝不落 step/日志/时间线/错误/API 响应**；`Registry.SetCredentialResolver` 统一注入。引用了凭据但未配密钥→执行**显式失败**（不放行无鉴权请求，不明文兜底）。**凭据管理端点（新增 `internal/credential` 包）**：`/credentials` CRUD——create/update 收明文 secret→加密落库（明文用完即弃）、list/get 只返元数据（密文 Sensitive 恒不回显，类似 API Key）；未配密钥时 create/update 返 **503**（拒绝明文兜底）；SEC-01 team 软隔离 + 建单鉴权（team 级用户须给可见 team 建）+ 审计留痕（只记名/类型/id，**明文/密文不入审计**）。**复用统一加密回填 T4.3（消除遗留明文缺口）**：T4.3 工单凭据此前仅 `.Sensitive()`（防回显但 DB 存明文），本轮 `ticket.Engine.SetCipher`/`Handler.SetCipher` 让工单凭据也走同一 cipher——写入加密、读取解密后传适配器；**向后兼容**：既有明文凭据解密失败时按明文透传，不破坏迁移前数据。**公共文件改动**：ent schema（`credential.go` + Team 反向边 + `runbook.go` StepTarget 加 `credential_ref`，`go generate`）；`config.go`（`Credential.EncryptionKey`）；`auth/permission.go`（+`credential.{view,create,update,delete}` 入 `AllPermissions`）；`auth/audit.go`（+3 审计 action）；`auth/scope.go`（注册 `credential`→team 反查）；`auth/seed.go`（team_admin 加 4 权限，org_admin 自动含）；`server/wire.go`（构造共享 `credCipher`[env 缺失记日志降级 nil，格式非法记 error 降级]+ 注入 runbook Registry/ticket Engine+Handler/credential Handler + 敏感路由权限点）；`.env.example`。OpenAPI spec + 前端 types 重生成（纯增量 +/credentials 端点）。测试锁定：加密往返/随机 nonce/错密钥/篡改/非法密文均 ErrDecrypt、空密钥 ErrKeyNotConfigured、非 32 字节 ErrInvalidKey、hex+base64 均可解析；执行时注入 bearer/token/header、**明文不进执行输出**、诊断探活也注入、无 cipher 引用即失败、无引用不注入、解密失败错误**不含明文**；凭据 DB 存密文（原始读回不含明文）、JSON/String() 脱敏、更新重加密；工单凭据加密存储+解密发出、**DB 无明文**、历史明文向后兼容透传。 |
| T6.4 多副本 WebSocket pub/sub | ✅（`014a0d0`） | **跨副本广播（`internal/ws/pubsub.go` + `hub.go`）**：多副本部署下用 Redis pub/sub 把任一实例的 WS 事件广播到所有副本，避免单实例限制；单副本退化不回归 + 跨副本去重（同一事件不重复推给已由本副本直推的连接）。装配层按 Redis 是否可用选实现（`server/wire.go`）。 |

> **注**：送达查询（`GET /incidents/:id/notifications`）已在 **T2.2** 落地（见 M2 进度节），T6.1 未重复。**M6 四个任务（T6.1–T6.4）已全部落地**：T6.1 报表 CSV 导出 + 定时聚合快照（`2fbea67`）、T6.2 平台化长尾（`2b5f311`，Event/RawEvent 保留清理[可配保留期+保护活跃关联+批量分页] + 服务拓扑 depends_on[schema+CRUD+一层查询] + 集成配置模板端点）、T6.3 Runbook 执行器凭据加密托管（`aad6a9c`，统一 `internal/crypto` AES-256-GCM + `Credential` 实体 + step `credential_ref` + 执行时解密注入 + 回填 T4.3 工单凭据）、T6.4 多副本 WS pub/sub（`014a0d0`）。其中 T6.2 内标注为设计目标暂缓的**完整向导 UI / 值班大屏 / PWA / WS 实时看板**属长尾前端/部署大件，归入文末「Phase 2 剩余工作」，不影响 M6 里程碑闭环。
...
## Phase 2 剩余工作（设计目标 / 长尾）

> Phase 1（M0–M6，39 任务）已全部合入 `main`，覆盖「功能可用、生产可交付」的核心闭环。
> 本节梳理系统扫描后**真实剩余**的设计目标与长尾项——多为大件（前端 UI、深水协同、外部 SDK、跨区调度、平台运维），不阻塞 Phase 1 交付，供后续版本排期。
> 与 [`backlog.md`](./backlog.md) 的关系：backlog 记录逐条推迟决定，本节按主题分类给全局视图。

### A. 明确推迟（不做）

- **作战室 War Room**（PRD M8.2 / M8.9 / M10.5）：自动建临时群、拉相关人、升级联动入群、关闭归档、消息回写时间线。编排复杂度高、平台能力差异大，需先做 IM 建群/群成员/消息回写 API 的 PoC。当前用「现有工作群 + 交互卡片 + 卡片实时刷新」替代。adapter 层 `CreateChat` 原语已保留，live path 未接。详见 [`backlog.md`](./backlog.md)。

### B. 前端 UI 缺口（后端端点已齐，缺 Web 页面）

- 凭据管理页（`/credentials` CRUD，T6.3 后端已就绪）
- 订阅管理页（`/subscriptions` 自管，T4.4 后端已就绪）
- 工单集成配置页（`/ticket-integrations` CRUD，T4.3 后端已就绪）
- 排班 Override 换班页（`/schedules/:id/overrides`，T2.3 后端已就绪）
- 通知规则 / 抑制规则管理页（T2.2 / T2.4 后端已就绪）
- IM 账号绑定 / 解绑页（旅程 B.9，T5.5 后端已就绪）
- 集成接入向导 UI（分步 wizard，T6.2 提供了 `config-template` 后端辅助端点）
- 值班大屏 / PWA、WS 实时看板（能力域 15 §B3，纯前端大件）

### C. 协同 / AI 深水区

- **Incident merge 端点**（M7）：重复单合并，当前阻塞 dedup accept 的完整闭环（去重建议可产出但无合并落地动作）。
- **跨团队事件级临时授权**（M8.3 / data-model §5.6）：`AddResponder` 目前仅把用户加入 responders 名单，**不发临时 RoleBinding**、关闭不自动失效。需事件级临时授权发放 + 生命周期回收。
- **工单双向回写**（能力域 14）：当前 T4.3 为单向（ActionItem→done 同步工单侧），工单侧状态变更反向拉回 Vigil 未做。
- **AI 噪声学习自动闭环**（M11.5）：T3.4 实现「记录 + 可查」（reject 建议按类型聚合成可观测指标），**不自动改变后续 AI 行为**（不回训、不自动生成 suppression 规则）——自动闭环待后续。

### D. IM / 接入长尾

- **企微完整 bot 适配器**：当前 `NoopBot` 占位（`Available()==false` 被排除，走 notification 兜底降级链不静默丢告警），完整适配器待 PoC。
- **IM 斜杠命令全量**（M8.5）：部分命令已实现，runbook / oncall 等命令仍缺。
- **Jira / 禅道 SDK 适配器**：T4.3 已有通用 webhook + `NewJiraAdapter`/`NewZentaoAdapter` 预留占位（返回 `ErrAdapterNotImplemented` 降级），完整 SDK 体量大待补。
- **webhook 出站动态订阅 CRUD**：当前出站 URL 靠 `VIGIL_WEBHOOK_OUT_URLS` 环境变量（全局静态），缺 per-订阅动态管理端点。
- **IaC / Terraform provider**（平台工程师旅程 P1-3）。

### E. 运维 / 平台长尾

- **migrate-down / 回滚**（H1.4）：当前无逆向迁移，回滚靠备份恢复。
- **首次部署向导**（first-run wizard）：当前靠环境变量 + 种子超管。
- **服务拓扑完整算法**：T6.2 已有 `depends_on` schema + CRUD + 一层依赖查询，**完整拓扑遍历 / 环检测 / 影响面传播**未做（标注设计目标）。
- **follow_the_sun 跨时区排班**：排班引擎已按 `Schedule.type` 差异化解算，跨时区 follow-the-sun 轮转策略待补。
- **用户禁用自动交接提示**（M13.1）：当前置 `status=disabled` 即时失效鉴权（T0.3）+ oncall 解算跳过禁用用户（T2.3），但不主动提示待交接的排班 / Action Item。
- **WS 实时看板**（能力域 15 §B3）：T6.4 已实现多副本 WS pub/sub 基础设施；看板级 dashboard 广播 topic + 前端订阅待补。

### F. 工程债

- **新功能 e2e 覆盖**：M1–M6 新增能力多以单测 + isolation 测试锁定，核心流水线（ingestion/triage/escalation/auth）e2e 已有，新功能（订阅/工单/凭据/保留清理/多副本 WS 等）e2e 覆盖待补。
- **B23 dedup Redis-nil 降级**：去重路径 Redis 不可用时的降级行为需复核加固。
- **凭据密钥接 KMS**：T6.3 密钥当前从 env `VIGIL_CREDENTIAL_ENCRYPTION_KEY` 读取，生产宜接 KMS / Vault 托管密钥轮换。
