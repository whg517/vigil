...
实体全貌以 `ent/schema/` 为准(26 个实体)。关键实体:User / Team / Member / Role / Permission / RoleBinding(身份与权限)、Service / Integration / Schedule / EscalationPolicy / Runbook / NotificationRule(配置蓝图)、Event / Incident / TimelineItem / IncidentAction / Postmortem / AIInsight(运行时实例)。
...
| 聚合 | 键 `service+severity`,5min 窗口内并入活跃 Incident;PostgreSQL 窗口函数 |
...
`msg.Channels` 是**有序降级链**(非并联):对每 target 逐通道尝试,首个成功即停。送达三态落库(`pending|sent|failed|suppressed`);整条链全失败→兜底告警 org_admin(走非 IM 通道)。30s 窗口聚合防轰炸,critical 立即单发;quiet_hours 支持 critical 穿透。

### 5.6 IM 协同层(ChatOps)★ 差异化核心 → [ADR-0018](./adr/0018-im-same-rbac-as-web.md) · [ADR-0019](./adr/0019-imbot-pluggable-degradation.md)

IM 双向通信 + 状态同步:IM 回调 → 映射 `im_accounts`→User → **走与 Web 完全相同的 RBAC 鉴权** → 调核心服务(复用 `internal/incident/service.go`)。交互卡片按权限渲染(无权按钮不显示),状态变化时原地更新卡片;平台能力参差走降级矩阵(飞书全 / 钉钉部分 / 企微 NoopBot 兜底不丢告警)。作战室自动建群拉人。
...
单二进制内按领域模块组织(`internal/` 下 37 个模块):
...
└── web/            # 前端 embed
ent/schema/         # 26 个实体定义(改后须 go generate ./ent/...)
...
| 风险 | 对策 |
|------|------|
| Redis 宕机 | Asynq 状态持久化 Redis(AOF/RDB + HA);接入层降级(先落 PostgreSQL 原始事件,恢复后回灌) |
...
- **集群**:Kubernetes + Helm(`deploy/helm/`),vigil-api(无状态多副本)与 vigil-worker(按队列深度扩缩)独立扩缩,状态全在 Redis,多实例 WebSocket 广播走 Redis pub/sub。
