...
**非功能基线**(目标值,来源 [ADR-0002](./adr/0002-product-positioning.md);压测方法与实测基线见
[operations.md §10 容量规划](./operations.md#10-容量规划压测方法与基线实测),回归验证依赖压测脚本、e2e 与 `/metrics`):

| 指标 | 目标 | 现状(2026-07) |
|------|------|------|
| 接入吞吐 | ≥ 1000 events/min | **已实测达标**:1000/min×3m 全量 202,归一化滞后 P95 1.3s;余量约 2 倍(2000/min 时归一化 P95 4.6s 逼近单实例消化极限) |
| 通知送达延迟 | P95 < 5s | 设计目标(**当前未实测**:压测环境无真实 IM/SMTP 通道;流水线内部 Incident→通知记录产生约 0.7s,通道投递延迟待配 IM 沙箱后补测) |
| 降噪效果 | 无意义打扰次数 ↓ 50%+(Event→通知的降噪率见 analytics) | 设计目标(运行期业务指标,由 analytics 持续度量,无法以合成压测断言;压测中聚合行为正常:3001 事件并为 4 单) |
| 部署门槛 | Docker Compose 一键起步(硬指标) | 已落地(见 operations.md §2) |
