...
实体全貌以 `ent/schema/` 为准(25 个实体 + `vector.go` 辅助类型)。关键实体:User / Team / Member / Role / Permission / RoleBinding(身份与权限)、Service / Integration / Schedule / EscalationPolicy / Runbook / NotificationRule(配置蓝图)、Event / Incident / TimelineItem / IncidentAction / Postmortem / AIInsight(运行时实例)。
...
└── web/            # 前端 embed
ent/schema/         # 25 个实体定义(改后须 go generate ./ent/...)
