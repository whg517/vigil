...
```
特性分支（feature/a、feature/b）在 worktree 内自由开发、频繁提交；
合入 main 时用 squash 压缩成一个提交，main 历史保持线性：

main ──●──●──●──●──●──●── (每个 ● 是一个特性 squash 后的提交，始终稳定)
            ↑     ↑
       feature/a  feature/b  (开发完即删，不入 main 历史)
```
...
- 验证顺序：开发 → worktree 内构建/测试全绿 → 合并。**不要先合并再在 main 上验证**——main 必须只接收已验证通过的改动。
...
git merge --squash feature/<name> # 把特性改动压到暂存区（不自动提交）
git commit -m "<规范的 commit message>"  # 用一条规范信息完成合并提交
git branch -D feature/<name>     # 删除已合并的特性分支
```
...
# 合入前验证（在 worktree 内，绿了才合并）
go build ./... && go test ./... && pnpm --dir web build

# 完成（回主目录 squash 合并）
cd /path/to/vigil
git merge --squash feature/<name>      # 压缩到暂存区（不自动提交）
git commit -m "feat(scope): 描述"       # 用一条规范信息完成 squash 提交
git worktree remove .worktree/feature-<name>
git branch -D feature/<name>            # squash 后用 -D（git 不认为已合并）
