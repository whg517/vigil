...
测试框架用 **Ginkgo + Gomega**（BDD 风格），位于顶层 `test/e2e/`（非 internal/，因为它是测试代码不是生产代码）。
...
```
test/e2e/
├── suite_test.go       # Ginkgo 入口：BeforeSuite 起完整实例（一次）+ AfterSuite 清理
├── helpers_test.go     # envState 方法 + fixture（seedTeam/Service/Integration/...）
│                        #   + gomega 轮询断言 + BeforeEach（resetDB + reseedAdmin）
...
**环境准备（BeforeSuite）**：整个 suite 只 bootstrap 一次 app 实例（之前每测试各起一次，
大半时间耗在重复装配）。`BeforeSuite` 做完：预分配端口 → 覆盖配置 → Bootstrap →
migrate → FlushDB → 启动 server/worker → 轮询 /health → 缓存 adminToken。
`BeforeEach` 只清数据（resetDB TRUNCATE + reseedAdmin 重建管理员 + 刷新 token），
`AfterSuite` 优雅关闭。这样 11 个 spec 共用一个实例，约 13s 跑完。

### 8.5 加新 e2e 用例的范式（Ginkgo）
...
var _ = Describe("某能力域", func() {
	Describe("某个行为", func() {
		It("期望的结果", func() {
			By("准备前置数据（BeforeEach 已清数据 + 重建 admin）")
			t := testEnv.seedTeam("团队")
			s := testEnv.seedService("svc", t.ID)
			_, integToken := testEnv.seedIntegration(adminToken, "prometheus", t.ID, s.ID)

			By("驱动业务（触发流水线/调 API）")
			testEnv.sendWebhook(integToken, promPayload(s.Slug, "fp-xxx"))

			By("轮询等待异步结果，断言库状态")
			incs := testEnv.waitForIncidentCount(1)
			Expect(incs[0].Status).To(Equal(incident.StatusTriggered))
		})
	})
})
```
...
关键约定：
- 全局 `testEnv` / `adminToken` 由 BeforeSuite/BeforeEach 管理，spec 内直接用。
- 异步结果用 **gomega Eventually**（封装在 `waitForXxx`），不要 sleep 固定时长。
- 数据隔离靠 `BeforeEach` 的 resetDB + flushRedis（清 dedup key/聚合器/asynq 残留）。
- Ginkgo dot-import（`Describe`/`It`/`Expect` 裸用）是官方惯例，已在 golangci-lint 豁免。
