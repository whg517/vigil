...
1. **告警源 Adapter** —— `Adapter.Normalize(raw) → []Event`。覆盖 Prometheus/Zabbix/Grafana/云监控/邮件/通用 webhook。
2. **通知通道 Channel** —— `Channel.Send(target, msg) → ack`。覆盖 IM / 电话·SMS / 邮件 / Webhook。
...
5. **IM Bot** —— `IMBot.*`(收发消息 / 卡片 / 建群)。钉钉 / 飞书 / 企微。
