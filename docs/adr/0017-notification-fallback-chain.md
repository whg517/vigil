...
- **可插拔与重试**:Notifier 可插拔(`Channel / Send`);失败走 Asynq 指数退避重试,幂等键 `notification_id`;模板用 Go `text/template`,内置 3 个默认模板 seed 幂等 upsert,渲染失败降级 `FormatTitle / FormatSummary` 兜底不丢通知。
