...
| 活文档 | 回答什么 | 与 ADR 的关系 |
|--------|---------|--------------|
| [`backlog.md`](../backlog.md) | 还剩什么没做、什么明确推迟(含重启前置) | 推迟裁决定型后可引用/升格为 ADR |
| [`design/`](../design/) | 一个新功能具体怎么做(docs-driven 的落笔处) | 设计中的关键取舍拆出为 ADR |
| [`operations.md`](../operations.md) | 部署/升级/备份/排查的可执行步骤 | 每步背后的"为什么"引用 ADR |
| [`known-issues.md`](../known-issues.md) | 已知未修缺陷与限制(带出处) | 修复后从清单移除 |
