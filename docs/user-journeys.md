...
| **· 排班与升级 ·** | | |
| 排班实时 oncall 计算 | ✅ | `schedule/engine.go` 实时算（**无 Redis 缓存**）；✅ 引擎**过滤 `status=active`**（禁用用户不被解算通知）；⚠️ `type` 中 calendar/rotation 走同套轮转算法（follow_the_sun 已单独实现，P3.2）（B.5/B.14） |
| Override 换班 | ✅ | Override 实体 + `POST/GET/DELETE /schedules/:id/overrides`；引擎窗口内顶替（`override=true`），换他人需 `schedule.update`（B.5.1） |
| 空班检测 | ✅ | 全层无人 → metric `vigil_schedule_empty_shifts_total` + Warn + 告警 team_admin（兜底 org_admin）（B.5.2） |
...
| **· Runbook ·** | | |
| Runbook 两档（readonly 自动 / 写操作人确认） | 🟡 | 写步骤强校验（须 `require_approval=true`）✅ + 引擎双保险（凡写步骤只认 approved）✅；未获批写步骤**按 on_failure 阻断**（continue 干跑/abort 中止/escalate 升级）✅；前端弹窗**真实传递审批决策**（复选框，不再恒 approved:true）✅；**执行/阻断/升级均记 actor**（时间线，source=web）✅；**execute 并发保护**（(runbook, incident) Redis 执行锁，approved 连点/并发第二次返回 409，只读干跑不锁）✅；⚠️ 仍：无独立审批人/双人复核、无 pending 审批流、AuditLog/IncidentAction 不写；执行器仅 http/internal（ansible/jenkins 📋）；trigger 不求值仅手动执行（B.8/C.5） |
| **· 复盘 ·** | | |
...
| migrate status | ✅ | 只读列出当前版本 / 已知版本（已应用/待应用 + 应用时间）/ 孤儿记录（D.1） |
| 回滚 | ✅ | **本项目不做版本化逆向迁移**（无 `migrate down`）：升级迁移失败一律通过**备份恢复**（`scripts/restore.sh`）回退，ent 实体结构变更同理靠备份恢复（D.1/D.2） |
...
```
   ...
   └───────────────────────────────────┘    │（API Key 生命周期，见 F.1–F.6）   │
            ...
            ┌──────────── 旅程 D：运维保障（长期运行，低频高危）────────┐
            │  升级/迁移（双轨 migrate；status 见 D.1，回滚靠备份恢复）    │
...
> ⚠️ 顺序缺陷（如实）：新代码先跑起来、migrate 后补——若新版依赖新列，窗口期内相关 API 报错。ent auto-migrate 是加法型变更（加表加列）时窗口影响小；破坏性变更须停机升级。回滚同 D.1：**本项目不做版本化逆向迁移，升级迁移失败一律通过备份恢复（`scripts/restore.sh`）回退**。
...
> 📋 设计目标：成员增删端点；移成员时联动处理其 RoleBinding 与 Rotation.participants
> （注：排班参与人本身已可经排班编辑表单 / `PATCH /schedules/:id` 提供 layers 重建 Rotation 管理，B.5；缺的是"移团队成员时自动联动清排班"这一步）。
     ...
     "layers": [                            # 分层（primary/secondary 用 priority 表达）
       {"name":"一线","priority":0,
        ...
        "rotation_type":"daily",            # daily | weekly | custom；非法值 400 "invalid layer 一线: invalid rotation_type"
                                            # ★ 决定轮换周期：daily=固定 24h、weekly=固定 168h、custom=用下方 shift_length
        "shift_length":"24h",               # ★ 仅 custom 生效（daily/weekly 忽略此字段）；支持 Go duration（"168h"）与 "1week"
...
5. 详情：GET /schedules/:id（✅ 回填每层 `participants` + 轮值配置，供编辑表单往返；`WithRotations(WithParticipants)`）
```

> ✅ **PATCH 提供 `layers` 会重建 Rotation（删旧建新，事务保证一致）**（`update()` B21，handler.go）。
> 即：**改参与人/班次参数可经 PATCH 完成**——无需删除整个 Schedule 重建。写用例时据此断言（提供 layers → 旧 Rotation 全删、按新 layers 重建）。
> 前端排班表单已提供**参与人多选 + 轮值配置（rotation_type/shift_length/handoff_time/start_date）管理入口**（`web/src/pages/oncall.tsx`），
> 编辑走 `GET /schedules/:id` 回填现有参与人与配置（避免 PATCH 缺 participants 把在班人清空）。

**计算算法**（`schedule/engine.go`，与 capability 03 §2.2 一致）：
班次序号 = `floor((T − start_date) / 周期)`，在班人 = `participants[序号 mod 人数]`，`handoff_time` 前沿用上一班。
**周期由 `rotation_type` 决定**（`shiftDurationFor`）：`daily`=固定 24h、`weekly`=固定 168h、`custom`=解析层的 `shift_length`。
即选 daily/weekly 时 `shift_length` 不参与计算，仅 `custom` 才取它。
时间按 **Schedule.timezone** 换算后计算；时区字符串非法时**降级 UTC 继续算，不报错**（跨时区验证时注意这一兜底）。

**Schedule.type 的真实语义（⚠️ 纠偏，勿与层的 `rotation_type` 混淆）**：`type`（`calendar` / `rotation` / `follow_the_sun`）三枚举可存库，
**引擎不因 `type` 切换轮转算法**——`calendar` / `rotation` 走同一套 Rotation 轮转；`follow_the_sun`（跨时区接力）已实现跨时区接力。
注意这里说的是 **Schedule.type**，与上文层内 `rotation_type`（决定轮换周期）是两回事。
...
> （按 priority 升序，snake_case），**不是**能力域文档写的 `{primary, secondary, overrides}`。`users[].override` 字段在 **Override 生效窗口内为 `true`**（顶班人那层，priority=-1 置顶），其余为 `false`（见 B.5.1）。
...
#### B.5.1 换班 Override 操作流 — ✅ 已实现（⚠️ 纠偏：曾标"完全未实现"）

> （附录 A"自己换班 Override"行已同步标 ✅。）

**当前行为（核对代码）**：
- **Override 实体存在**：`ent/schema/override.go` 定义 Override（schedule 边 + user 顶班人边 + created_by 边 + start_time/end_time/reason）；
- **写入口齐全**：`schedule/handler.go` 提供 `POST /schedules/:id/overrides`（建）、`GET /schedules/:id/overrides`（列，按 start_time 倒序）、`DELETE /schedules/:id/overrides/:oid`（删），`wire.go` 已装配（db 非 nil 时注册）；
- **引擎解算生效**：`engine.go` `resolveOverride` 查命中 `start_time <= at < end_time` 且顶班人在职（status=active）的 Override，命中则生成一个 priority=-1 的最高优先级 Override 层，`users[].override=true`，**完全覆盖** Rotation 结果（多条命中取最新创建）；
- **权限落地**：`schedule.override` 已被路由守卫校验；**换己班**（user_id==actor）仅需 `schedule.override`，**换他人**（user_id!=actor）额外要 `schedule.update`（团队管理员/超管才有），越权替别人换班返回 403（`requireOverrideOthers`）。
- **前端**：oncall.tsx 有"换班/Override"区（`OverrideSection`）——列出换班记录 + 新建（替班人下拉 + 起止时间 + 原因）+ 删除。

**行为要点（供验收）**：

```
POST /schedules/:id/overrides
{ "user_id": 44,                             # 顶班人（缺失/<=0 → 400 "user_id required"）
  "start_time": "2026-07-05T09:00:00+08:00", # RFC3339 必填（缺失 400）
  "end_time":   "2026-07-06T09:00:00+08:00", # 须晚于 start（否则 400 "end_time must be after start_time"）
  "reason": "张三请假" }
权限：schedule.override —— 换己班即可；换他人需 schedule.update（越权 403）
schedule 不存在 → 404；顶班人不存在/FK 冲突 → 400（非 500）
生效验证：窗口内 GET oncall → 顶部 Override 层 users[].override=true 且为顶班人；窗口外自动恢复原 Rotation 结果
顶班人被禁用：resolveOverride 过滤 status=active，禁用顶班人的 Override 不生效（回落原 Rotation）
与升级联动：升级触发实时解算 → Override 生效期间升级通知发给顶班人（无需改升级策略）
```

#### B.5.2 空班检测 — ✅ 已实现（⚠️ 纠偏：曾标"未实现"，与 B.14/C.7 旧断言相反）

> "空班 → 告警 team_admin"**已在代码中实现**。B.14/C.7 的相关表述已按此纠偏。

**当前实际行为**：
- `schedule/engine.go` `Oncall` 算完所有层后经 `isEmptyShift` 判定：**所有层都无在班人**（含无层）时判为空班 →
  记 metric `vigil_schedule_empty_shifts_total`（label=schedule_id）+ Warn 日志 + 调 `EmptyShiftAlerter.AlertEmptyShift`；
- 装配层 `wire.go` 注入 `emptyShiftAlerter`：解算该 Schedule 所属 **team 的在职 team_admin** → 走 `NotifyUnrouted`
  （email/phone/sms/webhook 独立通道，无单不走 IM 卡片）告警；**无 team 归属或该 team 无 team_admin 时兜底告警 org_admin**；best-effort，失败只记日志不放大故障；
- 单层空（某层无人但其它层有人）**不判为空班**——引擎 `continue` 跳过该空层，只要还有任一层有在班人就不触发告警；
- **可观测信号**：空班告警（team_admin/org_admin）+ metric + Warn 日志；升级侧时间线仍会记 `升级到 level N，通知 0 人`（TimelineItem detail `notified:0`）。

**空班的产生路径（枚举，供测试构造）**：

| 路径 | 结果 |
|------|------|
| layer 创建时不带 participants | 不建 Rotation → 该层为空；若是唯一层 → 整体空班触发告警 |
| Schedule 无任何带参与人的 layer | oncall 返回 `layers: []`（HTTP 200，空数组）→ 空班告警 team_admin |
| 查询时刻早于 start_date | **不空**——取 participants 首人（引擎兜底） |
| shift_length 写错（如 "1天"） | **不空**——解析失败降级默认 24h |
| 值班人全部被禁用（B.14） | **判为空班**——引擎过滤 `status=active`，禁用者不进解算，全禁用 → 该层空 → 触发空班告警 ✅ |

**兜底**：除引擎主动告警 team_admin 外，仍可叠加升级策略**末级 target=team** 的配置兜底（B.6 推荐末层"全团队"）——双保险。
...
| 所有 layer 无 participants | oncall 200 + `layers` 为空；触发空班告警 team_admin + metric +1；升级时间线记"通知 0 人" |
| 值班人全部被禁用 | 该层解算为空（过滤 status=active）→ 触发空班告警，禁用用户不被通知 ✅ |
| PATCH 提供 layers 换 participants | 旧 Rotation 全删、按新 layers 重建 → 在班人**随之改变**（B21，见上纠偏） |
| Override 窗口内 GET oncall | 顶部 Override 层（priority=-1）users[].override=true，为顶班人 |
...
| 排班通知 | ✅ **排班引擎过滤 status=active**（B.5.2 已核实）——禁用用户不再被解算为在班人，禁用后不再收到排班/升级中经排班解算的通知；若某层因此全空则触发空班告警 team_admin | ✅ 纠偏 |
...
1. 排班：把该用户移出 Rotation.participants
   ├─ ✅ 已有管理入口（B.5）——排班编辑表单改参与人，或 `PATCH /schedules/:id` 提供 layers 重建 Rotation（无需删除整个 Schedule）
   ├─ ✅ "建 Override 覆盖其班次"可行——Override 已实现（B.5.1），可临时指派替班人顶替其窗口
   └─ ✅ 纠偏：即使暂不移除，禁用本身也已"自动生效"——引擎过滤 status=active，
      禁用用户不再被解算为在班人；若某层因此全空，引擎会**触发空班告警 team_admin**（兜底 org_admin，B.5.2），
      不再"静默无人接手"。仍建议尽快清理排班使轮换恢复健康
...
- 禁用后（或经排班编辑/PATCH 移出参与人后）`GET /schedules/:id/oncall` 各层 users 不含该用户（引擎过滤 status=active）。
...
| 并发保护 | ✅ **(runbook, incident) 维度 Redis 执行锁**（`engine.go` `acquireExecLock`，SETNX 抢锁参考 triage dedup）：`approved=true` 时抢锁，已有执行在途 → 第二次返回 **409**（`ErrExecuteInProgress`），写步骤不重复触发；只读干跑（`approved=false`）不加锁、可反复预览；执行结束**主动释放锁**（允许合法重试），TTL（默认 10min）仅兜底进程崩溃。⚠️ 无 Redis 时降级为无锁（单机/测试；核心审批闸门仍在）；跨副本仍靠 Redis 单点，外部写端点自身幂等仍是最后一道保险 |
...
| 排班空班 | ✅ **告警 team_admin**——全层无人时 metric `vigil_schedule_empty_shifts_total` + Warn + 走独立通道通知 team_admin（无 team/无 TA 兜底 org_admin，B.5.2）；可再叠加升级策略**末级 target=team** 双保险 | ✅ |
...
# 输出：当前版本 / 已知版本清单（已应用●/待应用○ + 应用时间）/ 待应用数
# 「孤儿记录」= schema_migrations 有、但当前二进制嵌入目录已无对应文件的版本（提示降级/文件被删）
```

**回滚（★ 只走备份恢复，不做逆向迁移）**：

本项目**不提供 `migrate down` 版本化逆向迁移**。升级 / 迁移失败时的回滚**唯一手段是从备份恢复**（`scripts/restore.sh`，见 D.2/D.3）：
```
1. stop vigil
2. ./scripts/restore.sh backups/<timestamp>   # 恢复升级前的 PG + Redis 快照
3. 部署回旧版本镜像 → start vigil → 冒烟验证
```
- 之所以不做逆向迁移：ent auto-migrate 是声明式 diff，实体结构变更（建表/加列/改类型）无法安全自动逆向；与其维护脆弱的半自动 down 脚本，不如统一走「升级前必备份、失败即恢复」这条可靠路径。
- 因此 step 2a「先备份」是回滚能力的**唯一前提**，务必执行且定期在测试环境演练恢复。

**关键限制**：
- ❌ **无版本化逆向迁移**：任何结构 / 数据回退都靠备份恢复（见 D.2/D.3），这是为什么 step 2a 备份必做。
- ⚠️ `ent/schema/*.go` 改动后须 `go generate ./ent/...`（开发者责任，见 AGENTS.md）。
- ⚠️ ent auto-migrate 对**删列/改类型有限制**，破坏性变更须 hand-tuned SQL 挂到 `post_*.sql`；无法安全前向应用的破坏性变更应停机 + 备份保护下执行。
...
`GET /api/v1/schedules/{id}/oncall?time=`（✅）——把"现在谁值班"嵌进自家平台/群机器人。响应为 `{schedule_id, schedule_name, layers[]}`（⚠️ 非能力域文档的 `{primary, secondary, overrides}`，B.5 纠偏）；`GET /schedules/{id}/preview?days=` 拉未来排班。写操作（换班 Override）✅ 已实现——`POST/GET/DELETE /schedules/{id}/overrides`（B.5.1）。
...
| 换班 Override（`schedule.override`）| ✅ | — | — | — | ✅ | — | ✅ 已实现（B.5.1）；换己班需 `schedule.override`（org_admin/oncall 持有），换他人额外需 `schedule.update`（team_admin/超管） |
...
| 端点 | 权限 | 用途 / 关键行为 |
|------|------|------|
| `GET /schedules` (+`/:id`) · `POST` · `PATCH /:id` · `DELETE /:id` | GET 登录态；写 `schedule.create/update/delete` | 排班 CRUD（✅ PATCH 提供 layers **重建 Rotation**——改参与人经 PATCH 即可，无需删除重建；GET /:id 回填 participants，B.5） |
| `POST /schedules/:id/overrides` · `GET` · `DELETE /:id/overrides/:oid` | `schedule.override`（换他人另需 `schedule.update`） | ✅ 换班 Override（窗口内顶替，`override=true`，B.5.1） |
| `GET /schedules/:id/oncall?time=` | 登录态 | 实时查值班（返回 `{schedule_id,schedule_name,layers[]}`，Override 生效时顶部 override 层） |
...
| `POST /runbooks/:id/execute` | `runbook.execute` | 执行（写步骤需请求体 `approved:true`，否则 skip；无审批流；approved 执行有 (runbook, incident) 并发锁，连点/并发第二次 409，C.5.1） |
