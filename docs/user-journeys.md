...
| **· Runbook ·** | | |
| Runbook 两档（readonly 自动 / 写操作人确认） | 🟡 | 写步骤强校验（须 `require_approval=true`）✅ + 引擎双保险；⚠️ 但**引擎不校验 `require_approval` 标志**——"审批" = execute 请求体 `approved` 布尔（**请求方自报**，无审批流/无独立审批人/approved 零留痕）；**执行人不留痕**（时间线/审计皆无 who）；执行器仅 http/internal（ansible/jenkins 📋）；trigger 不求值仅手动执行；无并发保护（B.8/C.5） |
| **· 复盘 ·** | | |
...
| **· 报表 / 审计 / 治理 ·** | | |
| 数据报表 / 分析（6 端点） | ✅ | dashboard/alerts/incidents/team-load/postmortems/trend；⚠️ 响应键 PascalCase 与前端 camelCase 不一致（**Web KPI 恒空**，待修）；team-load/postmortem 指标缩水；无 export（B.11） |
...
#### 响应结构示例（键名从 engine.go 结构体抄）

> ⚠️ **键名为 Go 字段名（PascalCase）**：analytics 结构体**无 json tag**，echo 走 encoding/json 原样输出
> `Total`/`NoiseRate` 等键。但 **OpenAPI spec 与前端 types.gen.ts 是 camelCase**（`total`/`noiseRate`）——
> 两者不一致，前端仪表盘按 camelCase 取值会取到 undefined（KPI 恒显示 0/"—"），属实现缺陷（已记备忘/backlog）。
> **测试断言以运行时 PascalCase 为准。**
...
#### C.5.1 写操作审批的真实交互（⚠️ 纠偏：无弹窗审批流，"确认"= 请求体一个布尔字段）

原文"require_approval → IM/Web 弹窗确认，deny → skip/abort"是设计目标（capability 06 §5）。**实际实现**：
...
└─ target.readonly==false（写操作）
    ├─ approved==true  → 跑（引擎不看 require_approval——凡写步骤只认 approved，双保险见 B.8）
    └─ approved==false → skip（Skipped=true，error="write action requires approval, skipped"）
                          → 继续下一步（skip 不算失败、不触发 on_failure、不写时间线）
```

| 设计里的审批要素 | 实际情况（逐项核实 `runbook/engine.go` + `handler.go`） |
|------|------|
| 审批交互形态 | **同步请求参数**：无 pending 状态、无 409、无审批超时——approved 随 execute 请求一次性给定，缺省 false |
| Web 的"确认" | `/runbooks` 页执行弹窗即"确认"：一段警示文案 + [确认执行]，**恒发 `approved:true`**（想干跑 approved=false 只能 curl）；弹窗不展示哪些步骤是写操作 |
| IM 确认 | ❌ 无入口（`/vigil runbook` 403，C.3.5）——"写操作确认在 IM 完成"（M8）📋 |
| deny 分支 | **无 abort 选项**：approved=false 只会逐个 skip 写步骤、只读步骤照常执行（这也是合法的"干跑"用法，B.8 操作序列第 2 步）；"拒绝并中止"📋 |
| 确认者是谁 | **发起人自证**：唯一权限校验是发起人的 `runbook.execute`，approved 由同一人给出——无独立审批人、无双人复核（📋）；"admin 显式配 require_approval=false"路径实际不存在（数据层强制写步骤 require_approval=true，引擎也不读该标志） |
| 审批留痕 | ❌ 零留痕：approved 的值不进时间线不进审计——事后无法证明"谁确认的"（衔接 C.5.3） |
...
- ⚠️ **键名 PascalCase**（结构体无 json tag，与 B.11 analytics 同款缺陷）：前端按 `data.result` 取值恒 undefined →
...
**时间线（`runbook_executed`，✅ 每个非跳过步骤一条）**：内容「执行 Runbook 步骤 "X"」（失败追加"（失败）"），
detail=`{step,success,output}`；**跳过步骤不记**（干跑一次 approved=false 的验证在时间线上只见只读步骤）；
`incident_id=0`（不关联事件执行）则全程不记。
...
| `escalate` | 中止 + 触发升级 | 同 abort 的中止效果；另调 `incident.Service.Escalate(actorID=0, source=runbook)` ✅ **已接**——事件升一层、发 `IncidentEscalated` → escalation 立即通知下一层（B.6）。⚠️ 时间线上这条升级记录内容是「**系统 手动升级到 level N**」、source=**system**（timeline source 枚举无 runbook，映射进 default）——**看不出是 Runbook 触发的**，只能靠紧邻的失败步骤条目推断 |
...
- **用例锚点（3 步 runbook：只读→写→只读）**：① 全成功（approved=true）→ 3 条时间线、Aborted=false；
  ② 第 2 步失败 on_failure=continue → 3 条时间线（中间带失败）；③ 第 2 步失败 on_failure=escalate →
  2 条 runbook 时间线 + 1 条"系统 手动升级"+ 下一层收到通知；④ approved=false 干跑 → 响应 3 条 Steps（中间 Skipped）但时间线仅 2 条。
...
| when | ✅ 时间线 timestamp | — |
| **who** | ❌ **无处可查**——`RecordRunbook` 签名不含 actor，恒记 `actor={kind:system}`、source=system；HTTP 层也不写审计 | 🟡 **执行人身份全链路蒸发**：时间线、AuditLog（仍仅 7 种 action）、IncidentAction 三处都没有。"谁在生产上跑了写操作"这一最该审计的事实当前不可追溯（见开放问题 12） |
| 审批（approved）留痕 | ❌ 无（C.5.1） | 同上 |
...
**契约文档**：`GET /docs`（Swagger UI）+ `GET /openapi.yaml`（✅ 均已实现且**无鉴权公开**，`openapi.go`）。⚠️ 注意 spec 与运行时的已知偏差：analytics/Runbook 执行结果键名 PascalCase（B.11 / C.5.2），以 spec 为准会踩坑。
...
| 端点 | 权限 | 用途 / 关键行为 |
|------|------|------|
| `GET /analytics/dashboard` · `/alerts` · `/incidents` · `/team-load` · `/postmortems` · `/trend` | 登录态 ⚠️（无权限点、无团队 scope） | 6 个实时聚合端点（dashboard/trend `?days=`，其余 `?start=&end=` RFC3339；⚠️ 响应键 PascalCase，B.11） |
