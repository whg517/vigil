...
func (_q *UserQuery) QueryRoleBindings() *RoleBindingQuery {
...
func (_q *UserQuery) QueryAssignedIncidents() *IncidentQuery {
...
// QueryRespondingIncidents chains the current query on the "responding_incidents" edge.
func (_q *UserQuery) QueryRespondingIncidents() *IncidentQuery {
	query := (&IncidentClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *UserQuery) QueryRotations() *RotationQuery {
...
func (_q *UserQuery) WithRoleBindings(opts ...func(*RoleBindingQuery)) *UserQuery {
	query := (&RoleBindingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) WithAssignedIncidents(opts ...func(*IncidentQuery)) *UserQuery {
	query := (&IncidentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithRespondingIncidents tells the query-builder to eager-load the nodes that are connected to
// the "responding_incidents" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithRespondingIncidents(opts ...func(*IncidentQuery)) *UserQuery {
	query := (&IncidentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withRespondingIncidents = query
	return _q
}
...
func (_q *UserQuery) WithRotations(opts ...func(*RotationQuery)) *UserQuery {
	query := (&RotationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withRespondingIncidents; query != nil {
		if err := _q.loadRespondingIncidents(ctx, query, nodes,
			func(n *User) { n.Edges.RespondingIncidents = []*Incident{} },
			func(n *User, e *Incident) { n.Edges.RespondingIncidents = append(n.Edges.RespondingIncidents, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadRoleBindings(ctx context.Context, query *RoleBindingQuery, nodes []*User, init func(*User), assign func(*User, *RoleBinding)) error {
	...
	query.withFKs = true
	query.Where(predicate.RoleBinding(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.RoleBindingsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.user_role_bindings
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_role_bindings" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (_q *UserQuery) loadAssignedIncidents(ctx context.Context, query *IncidentQuery, nodes []*User, init func(*User), assign func(*User, *Incident)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	for _, n := range neighbors {
		fk := n.incident_assignee
		if fk == nil {
			return fmt.Errorf(`foreign-key "incident_assignee" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "incident_assignee" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *UserQuery) loadRespondingIncidents(ctx context.Context, query *IncidentQuery, nodes []*User, init func(*User), assign func(*User, *Incident)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*User)
	nids := make(map[int]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.RespondingIncidentsTable)
		s.Join(joinT).On(s.C(incident.FieldID), joinT.C(user.RespondingIncidentsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.RespondingIncidentsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.RespondingIncidentsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "responding_incidents" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (_q *UserQuery) loadRotations(ctx context.Context, query *RotationQuery, nodes []*User, init func(*User), assign func(*User, *Rotation)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*User)
	nids := make(map[int]map[*User]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.RotationsTable)
		s.Join(joinT).On(s.C(rotation.FieldID), joinT.C(user.RotationsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(user.RotationsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.RotationsPrimaryKey[1]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*Rotation](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "rotations" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
