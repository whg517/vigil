...
func (_q *SubscriptionQuery) QueryUser() *UserQuery {
...
func (_q *SubscriptionQuery) QueryTeam() *TeamQuery {
...
func (_q *SubscriptionQuery) QueryService() *ServiceQuery {
...
func (_q *SubscriptionQuery) WithUser(opts ...func(*UserQuery)) *SubscriptionQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *SubscriptionQuery) WithTeam(opts ...func(*TeamQuery)) *SubscriptionQuery {
	query := (&TeamClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SubscriptionQuery) WithService(opts ...func(*ServiceQuery)) *SubscriptionQuery {
	query := (&ServiceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(subscription.FieldChannels).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Subscription.Query().
//		Select(subscription.FieldChannels).
//		Scan(ctx, &v)
...
func (_q *SubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subscription, error) {
	...
	if _q.withUser != nil || _q.withTeam != nil || _q.withService != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withTeam; query != nil {
		if err := _q.loadTeam(ctx, query, nodes, nil,
			func(n *Subscription, e *Team) { n.Edges.Team = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withService; query != nil {
		if err := _q.loadService(ctx, query, nodes, nil,
			func(n *Subscription, e *Service) { n.Edges.Service = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *SubscriptionQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *User)) error {
...
func (_q *SubscriptionQuery) loadTeam(ctx context.Context, query *TeamQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *Team)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Subscription)
	for i := range nodes {
		if nodes[i].team_subscriptions == nil {
			continue
		}
		fk := *nodes[i].team_subscriptions
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "team_subscriptions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *SubscriptionQuery) loadService(ctx context.Context, query *ServiceQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *Service)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Subscription)
	for i := range nodes {
		if nodes[i].service_subscriptions == nil {
			continue
		}
		fk := *nodes[i].service_subscriptions
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "service_subscriptions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
