...
func (_q *NotificationQuery) QueryIncident() *IncidentQuery {
...
func (_q *NotificationQuery) WithIncident(opts ...func(*IncidentQuery)) *NotificationQuery {
	query := (&IncidentClient{config: _q.config}).Query()
...
func (_q *NotificationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Notification, error) {
	...
	if query := _q.withIncident; query != nil {
		if err := _q.loadIncident(ctx, query, nodes, nil,
			func(n *Notification, e *Incident) { n.Edges.Incident = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *NotificationQuery) loadIncident(ctx context.Context, query *IncidentQuery, nodes []*Notification, init func(*Notification), assign func(*Notification, *Incident)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Notification)
	for i := range nodes {
		if nodes[i].incident_notifications == nil {
			continue
		}
		fk := *nodes[i].incident_notifications
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "incident_notifications" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
