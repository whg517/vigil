...
func (_q *RoleQuery) QueryRoleBindings() *RoleBindingQuery {
...
func (_q *RoleQuery) WithRoleBindings(opts ...func(*RoleBindingQuery)) *RoleQuery {
	query := (&RoleBindingClient{config: _q.config}).Query()
...
func (_q *RoleQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Role, error) {
	...
	if query := _q.withRoleBindings; query != nil {
		if err := _q.loadRoleBindings(ctx, query, nodes,
			func(n *Role) { n.Edges.RoleBindings = []*RoleBinding{} },
			func(n *Role, e *RoleBinding) { n.Edges.RoleBindings = append(n.Edges.RoleBindings, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *RoleQuery) loadRoleBindings(ctx context.Context, query *RoleBindingQuery, nodes []*Role, init func(*Role), assign func(*Role, *RoleBinding)) error {
	...
	query.Where(predicate.RoleBinding(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(role.RoleBindingsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.role_role_bindings
		if fk == nil {
			return fmt.Errorf(`foreign-key "role_role_bindings" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "role_role_bindings" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
