...
func (_q *IntegrationQuery) QueryTeam() *TeamQuery {
...
func (_q *IntegrationQuery) QueryService() *ServiceQuery {
...
func (_q *IntegrationQuery) QueryRawEvents() *RawEventQuery {
...
func (_q *IntegrationQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *IntegrationQuery) FirstIDX(ctx context.Context) int {
...
func (_q *IntegrationQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *IntegrationQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *IntegrationQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *IntegrationQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *IntegrationQuery) WithTeam(opts ...func(*TeamQuery)) *IntegrationQuery {
	query := (&TeamClient{config: _q.config}).Query()
...
func (_q *IntegrationQuery) WithService(opts ...func(*ServiceQuery)) *IntegrationQuery {
	query := (&ServiceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *IntegrationQuery) WithRawEvents(opts ...func(*RawEventQuery)) *IntegrationQuery {
	query := (&RawEventClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *IntegrationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Integration, error) {
	...
	if _q.withTeam != nil || _q.withService != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withTeam; query != nil {
		if err := _q.loadTeam(ctx, query, nodes, nil,
			func(n *Integration, e *Team) { n.Edges.Team = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withService; query != nil {
		if err := _q.loadService(ctx, query, nodes, nil,
			func(n *Integration, e *Service) { n.Edges.Service = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withRawEvents; query != nil {
		if err := _q.loadRawEvents(ctx, query, nodes,
			func(n *Integration) { n.Edges.RawEvents = []*RawEvent{} },
			func(n *Integration, e *RawEvent) { n.Edges.RawEvents = append(n.Edges.RawEvents, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *IntegrationQuery) loadTeam(ctx context.Context, query *TeamQuery, nodes []*Integration, init func(*Integration), assign func(*Integration, *Team)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Integration)
	for i := range nodes {
		if nodes[i].team_integrations == nil {
			continue
		}
		fk := *nodes[i].team_integrations
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "team_integrations" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *IntegrationQuery) loadService(ctx context.Context, query *ServiceQuery, nodes []*Integration, init func(*Integration), assign func(*Integration, *Service)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Integration)
	for i := range nodes {
		if nodes[i].service_integrations == nil {
			continue
		}
		fk := *nodes[i].service_integrations
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "service_integrations" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *IntegrationQuery) loadRawEvents(ctx context.Context, query *RawEventQuery, nodes []*Integration, init func(*Integration), assign func(*Integration, *RawEvent)) error {
	...
	query.Where(predicate.RawEvent(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(integration.RawEventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.integration_raw_events
		if fk == nil {
			return fmt.Errorf(`foreign-key "integration_raw_events" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "integration_raw_events" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *IntegrationQuery) loadEvents(ctx context.Context, query *EventQuery, nodes []*Integration, init func(*Integration), assign func(*Integration, *Event)) error {
	...
	query.Where(predicate.Event(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(integration.EventsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.integration_events
		if fk == nil {
			return fmt.Errorf(`foreign-key "integration_events" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "integration_events" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *IntegrationQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(integration.Table, integration.Columns, sqlgraph.NewFieldSpec(integration.FieldID, field.TypeInt))
	_spec.From = _q.sql
