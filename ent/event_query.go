...
func (_q *EventQuery) QueryIntegration() *IntegrationQuery {
...
func (_q *EventQuery) QueryService() *ServiceQuery {
...
func (_q *EventQuery) QueryIncident() *IncidentQuery {
...
func (_q *EventQuery) WithIntegration(opts ...func(*IntegrationQuery)) *EventQuery {
	query := (&IntegrationClient{config: _q.config}).Query()
...
func (_q *EventQuery) WithService(opts ...func(*ServiceQuery)) *EventQuery {
	query := (&ServiceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *EventQuery) WithIncident(opts ...func(*IncidentQuery)) *EventQuery {
	query := (&IncidentClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//
//	client.Event.Query().
//		GroupBy(event.FieldSourceEventID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Event.Query().
//		Select(event.FieldSourceEventID).
//		Scan(ctx, &v)
...
func (_q *EventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Event, error) {
	...
	if _q.withIntegration != nil || _q.withService != nil || _q.withIncident != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withIntegration; query != nil {
		if err := _q.loadIntegration(ctx, query, nodes, nil,
			func(n *Event, e *Integration) { n.Edges.Integration = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withService; query != nil {
		if err := _q.loadService(ctx, query, nodes, nil,
			func(n *Event, e *Service) { n.Edges.Service = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withIncident; query != nil {
		if err := _q.loadIncident(ctx, query, nodes, nil,
			func(n *Event, e *Incident) { n.Edges.Incident = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *EventQuery) loadIntegration(ctx context.Context, query *IntegrationQuery, nodes []*Event, init func(*Event), assign func(*Event, *Integration)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Event)
	for i := range nodes {
		if nodes[i].integration_events == nil {
			continue
		}
		fk := *nodes[i].integration_events
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "integration_events" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *EventQuery) loadService(ctx context.Context, query *ServiceQuery, nodes []*Event, init func(*Event), assign func(*Event, *Service)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Event)
	for i := range nodes {
		if nodes[i].service_events == nil {
			continue
		}
		fk := *nodes[i].service_events
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "service_events" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *EventQuery) loadIncident(ctx context.Context, query *IncidentQuery, nodes []*Event, init func(*Event), assign func(*Event, *Incident)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Event)
	for i := range nodes {
		if nodes[i].event_incident == nil {
			continue
		}
		fk := *nodes[i].event_incident
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "event_incident" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
